...
test.describe('Reconciliation Workflows', () => {
  test('loads reconciliation endpoints and dashboard page', async ({ authenticatedPage, authToken }) => {
    ...
    const listResp = await authenticatedPage.request.get(`${apiURL}/api/v2/reconciliation`, { headers });
    expect([200, 204]).toContain(listResp.status());

    const dashboardResp = await authenticatedPage.request.get(`${apiURL}/api/v2/reconciliation/dashboard`, { headers });
