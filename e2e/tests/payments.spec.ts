...
test.describe('Payments Workflows', () => {
  test('supports config + intent validation + navigation persistence', async ({ authenticatedPage, authToken }) => {
    ...
    const config = await authenticatedPage.request.get(`${apiURL}/api/v2/payments/config`);
    expect(config.ok()).toBeTruthy();

    const invalidIntent = await authenticatedPage.request.post(`${apiURL}/api/v2/payments/intents`, {
      headers,
