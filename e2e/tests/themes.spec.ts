...
test.describe("Theming and Design System", () => {
  ...
  test("should toggle dark mode correctly", async ({ page }) => {
    ...
    const savedScheme = await page.evaluate(() =>
      localStorage.getItem("app-color-scheme"),
    );
    ...
    const savedScheme2 = await page.evaluate(() =>
      localStorage.getItem("app-color-scheme"),
    );
    expect(savedScheme2).toBe("light");
  });

  test("should persist theme across navigation and reload", async ({
    page,
  }) => {
  ...
  test("should handle quick theme cycling in navigation", async ({ page }) => {
    ...
    await expect
      .poll(
        async () =>
          page.evaluate(() => localStorage.getItem("app-theme") || ""),
        { timeout: 15000 },
      )
      .toBe(nextTheme);
  ...
  test("should apply high contrast theme for accessibility", async ({
    page,
  }) => {
  ...
  test("should keep primary navigation readable across representative theme and mode combinations", async ({
    page,
  }) => {
    const samples: Array<{
    ...
    for (const { theme, scheme } of samples) {
      ...
      const foreground = parseCssColor(navColors.color);
  ...
  test("should keep user dropdown opaque and readable", async ({ page }) => {
    ...
    const menuPanel = page
      .locator('div.menu-surface-opaque:has(a[href="/settings/user"])')
      .first();
    await expect(menuPanel).toBeVisible();

    const menuBgRaw = await menuPanel.evaluate(
  ...
  test("should keep the account menu theme controls opaque and readable in dark mode", async ({
    ...
    const panelBgRaw = await panel.evaluate(
      (el) => getComputedStyle(el).backgroundColor,
    );
  ...
  test("should keep People page filter options readable across representative themes and modes", async ({
    page,
  }) => {
    ...
    for (const { theme, scheme } of samples) {
      ...
      const selectColorRaw = await roleSelect.evaluate(
  ...
  test("should apply redesigned app tokens across every theme and mode pair", async ({
    page,
  }) => {
