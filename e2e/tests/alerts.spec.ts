...
test.describe('Alerts Workflows', () => {
  test('create and delete alert config with UI consistency check', async ({ authenticatedPage, authToken }) => {
    ...
    const listAfterCreate = await authenticatedPage.request.get(`${apiURL}/api/v2/alerts/configs`, { headers });
    ...
    const listAfterDelete = await authenticatedPage.request.get(`${apiURL}/api/v2/alerts/configs`, { headers });
