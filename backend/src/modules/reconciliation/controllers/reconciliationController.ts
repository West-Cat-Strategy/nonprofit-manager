...
export const getReconciliations = async (req: Request, res: Response): Promise<void> => {
  try {
    const validatedQuery = (req.validatedQuery ?? req.query) as ReconciliationQueryParams;
    const {
      status,
      reconciliation_type,
      start_date,
...
export const getReconciliationItems = async (
  ...
  try {
    const { id } = req.params;
    const query = (req.validatedQuery ?? req.query) as ReconciliationItemsQueryParams;
...
export const getAllDiscrepancies = async (req: Request, res: Response): Promise<void> => {
  try {
    const validatedQuery = (req.validatedQuery ?? req.query) as DiscrepancyQueryParams;
    const {
      status,
      severity,
      discrepancy_type,
