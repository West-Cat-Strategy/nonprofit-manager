...
export const DONATION_SELECT_COLUMNS = `
  ...
  d.designation_id,
  d.designation,
  COALESCE(fd.name, d.designation) as designation_label,
  fd.code as designation_code,
  fd.restriction_type as designation_restriction_type,
  d.is_recurring,
...
export const DONATION_RETURNING_COLUMNS = `
  ...
  appeal_campaign_id,
  campaign_name,
  (SELECT name FROM appeal_campaigns WHERE id = donations.appeal_campaign_id) as appeal_campaign_name,
  (SELECT code FROM appeal_campaigns WHERE id = donations.appeal_campaign_id) as appeal_campaign_code,
  (SELECT kind FROM appeal_campaigns WHERE id = donations.appeal_campaign_id) as appeal_campaign_kind,
  (SELECT status FROM appeal_campaigns WHERE id = donations.appeal_campaign_id) as appeal_campaign_status,
  designation_id,
  designation,
  COALESCE((SELECT name FROM fund_designations WHERE id = donations.designation_id), designation) as designation_label,
  (SELECT code FROM fund_designations WHERE id = donations.designation_id) as designation_code,
  (SELECT restriction_type FROM fund_designations WHERE id = donations.designation_id) as designation_restriction_type,
  is_recurring,
  recurring_frequency,
