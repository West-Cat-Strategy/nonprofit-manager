...
const getOrganizationId = (req: AuthRequest, res: Response): string | null => {
...
export const reorderAgenda = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const organizationId = getOrganizationId(req, res);
    if (!organizationId) return;
    await meetingService.reorderAgendaItems(
      req.params.id,
      req.body.orderedIds,
      req.user.id,
      organizationId
...
export const getMinutesDraft = async (
  req: AuthRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
