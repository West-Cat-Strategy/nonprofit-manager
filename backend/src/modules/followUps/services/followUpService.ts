...
import {
  ...
  mapFollowUpRow,
  upsertNotificationForFollowUp,
} from './followUpService.helpers';
import { type FollowUpQueryRow } from './followUpService.helpers';
...
export class FollowUpService {
  ...
  async getFollowUps(
    ...
    return {
      data: rowsResult.rows.map(mapFollowUpRow),
      pagination: {
        page,
        limit,
        total,
  ...
  async getFollowUpById(
    ...
    return mapFollowUpRow(result.rows[0]);
  }
  ...
  async createFollowUp(
    ...
    await upsertNotificationForFollowUp(this.db, result.rows[0].id);
    return mapFollowUpRow(result.rows[0]);
  }
  ...
  async updateFollowUp(
    ...
    if (result.rows.length === 0) return null;
    await upsertNotificationForFollowUp(this.db, followUpId);
    return mapFollowUpRow(result.rows[0]);
  }
      ...
      if (shouldScheduleNext) {
        ...
        if (nextDate) {
          ...
          if (!endDate || candidate <= endDate) {
            ...
            await upsertNotificationForFollowUp(this.db, insertedResult.rows[0].id);
          }
        }
      }

      await client.query('COMMIT');
      return mapFollowUpRow(updateResult.rows[0]);
    } catch (error) {
      await client.query('ROLLBACK');
