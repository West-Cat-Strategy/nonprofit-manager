...
import {
  ...
  validateWebhookUrl,
  type WebhookSendRequestOptions,
  type WebhookSendRequestResult,
...
interface ClaimedRetryDeliveryRow {
  id: string;
...
async function attemptDelivery({
  ...
  payload,
  existingAttempts,
}: AttemptDeliveryInput): Promise<void> {
  ...
  for (const row of rows) {
    ...
    try {
      await attemptDelivery({
        ...
        endpoint,
        payload: row.payload,
        existingAttempts: Number(row.attempts) || 0,
      });
      processed += 1;
    } catch (error) {
...
export type { WebhookSendRequestOptions, WebhookSendRequestResult };
