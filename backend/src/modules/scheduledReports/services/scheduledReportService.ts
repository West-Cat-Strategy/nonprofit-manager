...
} from './scheduledReportMappers';
import {
  computeNextRunAt,
  validateScheduleFields,
  validateTimezone,
} from './scheduledReportScheduling';
...
export class ScheduledReportService {
  ...
  async listScheduledReportRuns(
    ...
    return result.rows.map(mapScheduledReportRun);
  }
  ...
  async createRunRecord(
    ...
    return mapScheduledReportRun(result.rows[0]);
  }
      ...
      if (report.recipients.length === 0) {
        ...
        const delivered = await sendMail({
          ...
          text: `Your scheduled report "${report.name}" is attached.\n\nRows: ${
            exportJob.rowsCount ?? 0
          }`,
          html: `<p>Your scheduled report <strong>${report.name}</strong> is attached.</p><p>Rows: ${
            exportJob.rowsCount ?? 0
          }</p>`,
          attachments: [
            {
