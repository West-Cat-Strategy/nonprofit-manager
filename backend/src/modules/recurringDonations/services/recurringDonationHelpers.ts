...
export const PLAN_SELECT = `
  ...
  rdp.designation_id,
  rdp.designation,
  COALESCE((SELECT fd.name FROM fund_designations fd WHERE fd.id = rdp.designation_id), rdp.designation) AS designation_label,
  (SELECT fd.code FROM fund_designations fd WHERE fd.id = rdp.designation_id) AS designation_code,
  (SELECT fd.restriction_type FROM fund_designations fd WHERE fd.id = rdp.designation_id) AS designation_restriction_type,
  rdp.notes,
  rdp.status,
...
export const mapPlanRow = (row: PlanRow | undefined | null): RecurringDonationPlan | null => {
  ...
  return {
    ...
    designation_id: (row.designation_id as string | null) ?? null,
    designation: (row.designation as string | null) ?? null,
    designation_label: (row.designation_label as string | null) ?? null,
    designation_code: (row.designation_code as string | null) ?? null,
    designation_restriction_type:
      (row.designation_restriction_type as RecurringDonationPlan['designation_restriction_type']) ??
      null,
