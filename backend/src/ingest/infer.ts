...
export function inferColumn(values: Array<string | null>): {
  ...
  for (const s of nonNull) {
    ...
    if (/[ۣ$]/.test(s)) hasCurrencySymbol = true;
    if (/\d,\d{3}\b/.test(s)) hasCommaNumber = true;
  ...
  if (hasCurrencySymbol) patterns.push('contains_currency_symbol');
  if (hasCommaNumber) patterns.push('contains_thousands_separators');
  ...
  const confidence = clamp(
    inferredType === 'string' ? 0.5 : ratio((stats.counts[inferredType] as number) ?? 0),
    0,
    1
  );
...
export function inferColumnType(values: Array<string | null>): InferredType {
