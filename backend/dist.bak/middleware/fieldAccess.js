...
exports.filterFieldAccess = filterFieldAccess;
exports.checkFieldWriteAccess = checkFieldWriteAccess;
exports.requirePermission = requirePermission;
exports.logSensitiveFieldAccess = logSensitiveFieldAccess;
exports.clearFieldAccessCache = clearFieldAccessCache;
 ...
 */
async function getFieldAccessRules(userId, resource) {
    ...
    `;
        const result = await database_1.default.query(query, [userId, resource]);
        const rules = result.rows;
        fieldAccessCache.set(cacheKey, { rules, timestamp: Date.now() });
        return rules;
    }
    catch (error) {
        logger_1.logger.error('Error fetching field access rules', { error, userId, resource });
        return [];
    }
}
/**
...
async function hasPermission(userId, permissionName) {
    ...
    `;
        const result = await database_1.default.query(query, [userId, permissionName]);
        return result.rows[0]?.has_permission || false;
    }
    catch (error) {
        logger_1.logger.error('Error checking permission', { error, userId, permissionName });
        return false;
    }
}
/**
...
function applyMask(value, maskType) {
    ...
    switch (maskType) {
        case 'email':
            return (0, encryption_1.maskEmail)(strValue);
        case 'phone':
            return (0, encryption_1.maskPhone)(strValue);
        case 'partial':
            return (0, encryption_1.maskData)(strValue, 4);
        case 'ssn':
            return (0, encryption_1.maskData)(strValue, 4, '*');
        case 'full':
            return '********';
 ...
 */
function filterRecord(record, rules, isAdmin) {
    if (isAdmin) {
    ...
    for (const [key, value] of Object.entries(record)) {
        ...
        if (!rule) {
            ...
            if (rule.mask_on_read && value !== null && value !== undefined) {
                ...
                if (typeof value === 'string' && (0, encryption_1.isEncrypted)(value)) {
                    try {
                        filtered[key] = (0, encryption_1.decrypt)(value);
                    }
                    catch {
                        filtered[key] = value;
                    }
 ...
 */
function filterFieldAccess(resource) {
        ...
        res.json = function (data) {
            (async () => {
                try {
                    ...
                    if (rules.length === 0 && DEFAULT_DENY_ON_NO_RULES) {
                        logger_1.logger.warn('No field access rules found; default deny applied', { userId, resource });
                    }
                    // Filter the data
                    let filtered;
                    if (Array.isArray(data)) {
                        // Array of records
                        filtered = data.map((record) => filterRecord(record, rules, isAdmin));
                    }
                        ...
                        if ('data' in objData && Array.isArray(objData.data)) {
                            filtered = {
                                ...objData,
                                data: objData.data.map((record) => filterRecord(record, rules, isAdmin)),
                            };
                        }
                        else {
                            // Single record
                    ...
                    logger_1.logger.error('Error filtering field access', { error, resource });
                    return originalJson(data);
                }
            })();
            return res;
 ...
 */
function checkFieldWriteAccess(resource, fields) {
    return async (req, res, next) => {
        try {
            const userId = req.user?.id;
            const isAdmin = req.user?.role === 'admin';
            ...
            if (deniedFields.length > 0) {
                logger_1.logger.warn('Field write access denied', {
                    userId,
                    resource,
                    deniedFields,
                });
            ...
            logger_1.logger.error('Error checking field write access', { error, resource });
...
function requirePermission(permissionName) {
    return async (req, res, next) => {
        try {
            ...
            if (!permitted) {
                logger_1.logger.warn('Permission denied', { userId, permissionName });
                return res.status(403).json({
                    error: 'Forbidden',
            ...
            logger_1.logger.error('Error checking permission', { error, permissionName });
            return res.status(500).json({ error: 'Internal server error' });
        }
    };
}
 ...
 */
async function logSensitiveFieldAccess(userId, resourceType, resourceId, fieldName, accessType, req) {
    try {
        const query = `
    ...
    `;
        ...
        logger_1.logger.error('Error logging sensitive field access', { error });
    }
}
/**
 * Clear field access cache (for when permissions are updated)
 */
function clearFieldAccessCache(userId) {
    if (userId) {
