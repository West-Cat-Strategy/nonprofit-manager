...
exports.fetchStripeBalanceTransactions = fetchStripeBalanceTransactions;
 ...
 */
function generateReconciliationNumber() {
    const date = new Date();
 ...
 */
async function fetchStripeBalanceTransactions(startDate, endDate) {
    ...
    try {
        ...
        logger_1.logger.info(`Fetched ${transactions.length} Stripe balance transactions`, {
            startDate,
            endDate,
        });
        return transactions;
        ...
        logger_1.logger.error('Error fetching Stripe balance transactions:', error);
        throw error;
    }
}
/**
 ...
 */
async function saveStripeBalanceTransactions(transactions, reconciliationId) {
    for (const tx of transactions) {
        const data = {
            stripe_balance_transaction_id: tx.id,
            stripe_source_id: tx.source,
 ...
 */
async function fetchDonationsForReconciliation(startDate, endDate) {
 ...
 */
function matchTransactions(donations, stripeTransactions) {
    const matched = [];
    ...
    for (let i = unmatchedDonations.length - 1; i >= 0; i--) {
        ...
        if (donation.stripe_payment_intent_id || donation.stripe_charge_id) {
            const txIndex = chargeTransactions.findIndex((tx) => {
                ...
                return (sourceId === donation.stripe_payment_intent_id ||
 ...
 */
async function createReconciliationItems(reconciliationId, matched, unmatchedDonations, unmatchedStripe) {
 ...
 */
async function createDiscrepancies(reconciliationId) {
 ...
 */
async function createReconciliation(request, userId) {
    if (!isStripeConfigured()) {
        throw new Error('Stripe is not configured');
    }
    ...
    try {
        ...
        const reconciliationResult = await database_1.default.query(`
    ...
    `, [reconciliationNumber, request.reconciliation_type, startDate, endDate, userId, request.notes]);
        ...
        logger_1.logger.info(`Starting reconciliation ${reconciliation.id}`, {
    ...
    `, [
        ...
        logger_1.logger.info(`Completed reconciliation ${reconciliation.id}`, {
 ...
 */
async function getReconciliationItems(reconciliationId, matchStatus) {
 ...
 */
async function getDiscrepancies(reconciliationId) {
    const result = await database_1.default.query(`
 ...
 */
async function getReconciliationSummary(reconciliationId) {
  ...
  `, [reconciliationId]);
    ...
    return {
        ...
        total_net_amount: (parseFloat(reconciliation.stripe_balance_amount) || 0) - (parseFloat(reconciliation.stripe_total_fees) || 0),
 ...
 */
async function manualMatch(donationId, stripePaymentIntentId, userId) {
  ...
  `, [stripePaymentIntentId, userId, donationId]);
    logger_1.logger.info(`Manually matched donation ${donationId} to Stripe payment ${stripePaymentIntentId}`);
}
/**
 * Resolve a discrepancy
 */
async function resolveDiscrepancy(discrepancyId, status, resolutionNotes, userId) {
    await database_1.default.query(`
  ...
  `, [status, resolutionNotes, userId, discrepancyId]);
    logger_1.logger.info(`Resolved discrepancy ${discrepancyId} with status ${status}`);
}
