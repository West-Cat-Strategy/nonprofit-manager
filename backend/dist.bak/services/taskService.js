...
class TaskService {
    ...
    async getTasks(filters) {
        const { search, status, priority, assigned_to, related_to_type, related_to_id, due_before, due_after, overdue, page = 1, limit = 20, } = filters;
    ...
    `;
        ...
        const summary = {
            ...
            by_status: {
                [task_1.TaskStatus.NOT_STARTED]: parseInt(summaryRow.not_started),
                [task_1.TaskStatus.IN_PROGRESS]: parseInt(summaryRow.in_progress),
                [task_1.TaskStatus.WAITING]: parseInt(summaryRow.waiting),
                [task_1.TaskStatus.COMPLETED]: parseInt(summaryRow.completed),
                [task_1.TaskStatus.DEFERRED]: parseInt(summaryRow.deferred),
                [task_1.TaskStatus.CANCELLED]: parseInt(summaryRow.cancelled),
            },
            by_priority: {
                [task_1.TaskPriority.LOW]: parseInt(summaryRow.priority_low),
                [task_1.TaskPriority.NORMAL]: parseInt(summaryRow.priority_normal),
                [task_1.TaskPriority.HIGH]: parseInt(summaryRow.priority_high),
                [task_1.TaskPriority.URGENT]: parseInt(summaryRow.priority_urgent),
            },
    ...
    async createTask(taskData, userId) {
        const { subject, description, status = task_1.TaskStatus.NOT_STARTED, priority = task_1.TaskPriority.NORMAL, due_date, assigned_to, related_to_type, related_to_id, } = taskData;
        const query = `
      INSERT INTO tasks (
    ...
    async updateTask(id, updates, userId) {
        ...
        if (updates.status === task_1.TaskStatus.COMPLETED && !updates.completed_date) {
            updates.completed_date = new Date().toISOString();
        }
        // Handle status change from completed to other status
        if (updates.status && updates.status !== task_1.TaskStatus.COMPLETED) {
...
exports.taskService = new TaskService(database_1.default);
