...
class EventService {
     ...
     */
    async getEvents(filters = {}, pagination = {}, scope) {
        ...
        const { sortColumn, sortOrder } = (0, queryHelpers_1.resolveSort)(sort_by, sort_order, sortColumnMap, 'start_date');
        const dataQuery = `
      SELECT 
        id as event_id,
        name as event_name,
     ...
     * Get event by ID
     */
    async getEventById(eventId, scope) {
        const query = `
      SELECT 
        id as event_id,
        name as event_name,
    ...
    `;
        ...
        const finalQuery = conditions.length > 0 ? `${query} AND ${conditions.join(' AND ')}` : query;
     ...
     */
    async getEventAttendanceSummary(referenceDate = new Date(), scope) {
    ...
    `;
      ...
      WHERE start_date >= $1
        AND start_date <= $2
    `;
        const scopeCondition = scope?.createdByUserIds && scope.createdByUserIds.length > 0
        ...
        const upcomingParams = [referenceDate];
        const monthParams = [startOfMonth, endOfMonth];
        if (scope?.createdByUserIds && scope.createdByUserIds.length > 0) {
    ...
    async createEvent(eventData, userId) {
        const { event_name, description, event_type, status = 'planned', start_date, end_date, location_name, address_line1, address_line2, city, state_province, postal_code, country, capacity, } = eventData;
        const query = `
      INSERT INTO events (
     ...
     */
    async registerContact(registrationData) {
    ...
    `;
        ...
        await this.pool.query('UPDATE events SET registered_count = registered_count + 1 WHERE id = $1', [event_id]);
        return result.rows[0];
    }
    /**
     * Update registration
    ...
    `;
        ...
        // Update event registered count
        await this.pool.query('UPDATE events SET registered_count = GREATEST(registered_count - 1, 0) WHERE id = $1', [eventId]);
    }
}
exports.EventService = EventService;
exports.default = new EventService(database_1.default);
