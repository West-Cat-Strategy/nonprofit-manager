...
exports.TrendAnalyticsService = void 0;
const logger_1 = require("../../config/logger");
const redis_1 = require("../../config/redis");
const donationAnalytics_1 = require("./donationAnalytics");
const volunteerAnalytics_1 = require("./volunteerAnalytics");
const eventAnalytics_1 = require("./eventAnalytics");
class TrendAnalyticsService {
    constructor(pool) {
        this.pool = pool;
        this.donationAnalytics = new donationAnalytics_1.DonationAnalyticsService(pool);
        this.volunteerAnalytics = new volunteerAnalytics_1.VolunteerAnalyticsService(pool);
        this.eventAnalytics = new eventAnalytics_1.EventAnalyticsService(pool);
    }
    /**
     ...
     */
    async getTrendAnalysis(metricType, months = 12) {
        try {
            ...
            // Cache for 1 hour
            await (0, redis_1.setCached)(cacheKey, result, 3600);
            return result;
        }
        catch (error) {
            logger_1.logger.error('Error analyzing trends', { error, metricType });
     ...
     */
    async detectAnomalies(metricType, months = 12, sensitivityStdDev = 2.0) {
        try {
            ...
            logger_1.logger.error('Error detecting anomalies', { error, metricType });
    ...
    async getComparativeAnalytics(periodType = 'month') {
        try {
            ...
            const cached = await (0, redis_1.getCached)(cacheKey);
            if (cached) {
                logger_1.logger.debug('Comparative analytics cache hit', { cacheKey });
                return cached;
            }
            const now = new Date();
            let currentStart;
      ...
      `;
            ...
            const result = {
                ...
                metrics: {
                    total_donations: calculateComparison(currentDonations ? parseFloat(currentDonations.total_amount) : 0, previousDonations ? parseFloat(previousDonations.total_amount) : 0),
                    donation_count: calculateComparison(currentDonations ? parseInt(currentDonations.count) : 0, previousDonations ? parseInt(previousDonations.count) : 0),
                    average_donation: calculateComparison(currentDonations ? parseFloat(currentDonations.average_amount) : 0, previousDonations ? parseFloat(previousDonations.average_amount) : 0),
                    new_contacts: calculateComparison(currentContacts ? parseInt(currentContacts.count) : 0, previousContacts ? parseInt(previousContacts.count) : 0),
                    total_events: calculateComparison(currentEvents ? parseInt(currentEvents.count) : 0, previousEvents ? parseInt(previousEvents.count) : 0),
                    volunteer_hours: calculateComparison(currentVolunteer ? parseFloat(currentVolunteer.hours) : 0, previousVolunteer ? parseFloat(previousVolunteer.hours) : 0),
                    engagement_score: calculateComparison(0, 0), // Placeholder for now
                },
            };
            // Cache for 10 minutes (600 seconds)
            await (0, redis_1.setCached)(cacheKey, result, 600);
            return result;
        }
        catch (error) {
            logger_1.logger.error('Error getting comparative analytics', { error });
            throw new Error('Failed to retrieve comparative analytics');
        }
    }
}
exports.TrendAnalyticsService = TrendAnalyticsService;
//# sourceMappingURL=trendAnalytics.js.map