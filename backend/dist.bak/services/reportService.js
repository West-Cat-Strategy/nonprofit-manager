...
class ReportService {
     ...
     */
    buildWhereClause(filters, fieldSpecs) {
     ...
     */
    buildOrderByClause(sort, fieldSpecs) {
     ...
     */
    async generateReport(definition) {
        ...
        try {
            ...
            const selectFields = definition.fields
                ...
                .join(', ');
            // Build WHERE clause
            const { clause: whereClause, values } = this.buildWhereClause(definition.filters || [], fieldSpecs);
        ...
        ${limitClause}
      `.trim();
            logger_1.logger.debug('Executing report query', { query, values });
            // Execute query
      ...
      `.trim();
            ...
            logger_1.logger.error('Error generating report', { error, definition });
