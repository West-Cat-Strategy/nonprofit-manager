...
class DonationService {
     ...
     */
    async getDonations(filters = {}, pagination = {}, scope) {
        const { search, account_id, contact_id, payment_method, payment_status, campaign_name, is_recurring, min_amount, max_amount, start_date, end_date, } = filters;
    ...
    `;
        ...
        const { sortColumn, sortOrder } = (0, queryHelpers_1.resolveSort)(sort_by, sort_order, sortColumnMap, 'donation_date');
     ...
     * Get donation by ID
     */
    async getDonationById(donationId, scope) {
        const query = `
      SELECT 
        d.id as donation_id,
        d.donation_number,
    ...
    async createDonation(donationData, userId) {
        const { account_id, contact_id, amount, currency = 'USD', donation_date, payment_method, payment_status = 'pending', transaction_id, campaign_name, designation, is_recurring = false, recurring_frequency, notes, } = donationData;
     ...
     * Update donation
     */
    async updateDonation(donationId, donationData, userId) {
        const fields = [];
        const values = [];
        let paramCount = 1;
     ...
     */
    async markReceiptSent(donationId, userId) {
        const query = `
      UPDATE donations
      SET 
        receipt_sent = true,
     ...
     */
    async getDonationSummary(filters = {}, scope) {
        const conditions = [];
        const params = [];
        let paramCount = 1;
...
exports.default = new DonationService(database_1.default);
