...
class SiteManagementService {
    ...
    async createSite(userId, data) {
        ...
        const templateResult = await this.pool.query('SELECT id FROM templates WHERE id = $1 AND (user_id = $2 OR is_system_template = TRUE)', [templateId, userId]);
        if (templateResult.rows.length === 0) {
            throw new Error('Template not found or access denied');
        }
        // Check subdomain availability if provided
        if (subdomain) {
            const subdomainCheck = await this.pool.query('SELECT id FROM published_sites WHERE subdomain = $1', [subdomain.toLowerCase()]);
        ...
        if (customDomain) {
            const domainCheck = await this.pool.query('SELECT id FROM published_sites WHERE custom_domain = $1', [customDomain.toLowerCase()]);
            if (domainCheck.rows.length > 0) {
                throw new Error('Custom domain is already in use');
            }
        }
        const result = await this.pool.query(`INSERT INTO published_sites (
    ...
    async getSite(siteId, userId) {
        const result = await this.pool.query('SELECT * FROM published_sites WHERE id = $1 AND user_id = $2', [siteId, userId]);
    ...
    async getSiteBySubdomain(subdomain) {
        const result = await this.pool.query('SELECT * FROM published_sites WHERE subdomain = $1 AND status = $2', [subdomain.toLowerCase(), 'published']);
        if (result.rows.length === 0) {
            return null;
        }
        return this.mapRowToSite(result.rows[0]);
    ...
    async getSiteByDomain(domain) {
        const result = await this.pool.query('SELECT * FROM published_sites WHERE custom_domain = $1 AND status = $2', [domain.toLowerCase(), 'published']);
        if (result.rows.length === 0) {
            return null;
        }
        return this.mapRowToSite(result.rows[0]);
     ...
     */
    async updateSite(siteId, userId, data) {
        ...
        if (data.subdomain !== undefined) {
            // Check availability
            if (data.subdomain) {
                const subdomainCheck = await this.pool.query('SELECT id FROM published_sites WHERE subdomain = $1 AND id != $2', [data.subdomain.toLowerCase(), siteId]);
                if (subdomainCheck.rows.length > 0) {
                    throw new Error('Subdomain is already taken');
                }
            }
        ...
        if (data.customDomain !== undefined) {
            ...
            if (data.customDomain) {
                const domainCheck = await this.pool.query('SELECT id FROM published_sites WHERE custom_domain = $1 AND id != $2', [data.customDomain.toLowerCase(), siteId]);
                if (domainCheck.rows.length > 0) {
                    throw new Error('Custom domain is already in use');
                }
            }
        ...
        values.push(siteId);
        values.push(userId);
        const result = await this.pool.query(`UPDATE published_sites SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP
    ...
    async deleteSite(siteId, userId) {
        const result = await this.pool.query('DELETE FROM published_sites WHERE id = $1 AND user_id = $2 RETURNING id', [siteId, userId]);
        return result.rows.length > 0;
    }
    /**
     * Search published sites for a user
     */
    async searchSites(userId, params) {
        const { status, search, page = 1, limit = 10, sortBy = 'createdAt', sortOrder = 'desc', } = params;
        ...
        // Get total count
        const countResult = await this.pool.query(`SELECT COUNT(*) FROM published_sites WHERE ${whereClause}`, values);
        const total = parseInt(countResult.rows[0].count, 10);
        // Get paginated results
        const offset = (page - 1) * limit;
        const result = await this.pool.query(`SELECT * FROM published_sites
       WHERE ${whereClause}
    ...
    mapRowToSite(row) {
        return {
            ...
            sslCertificateExpiresAt: row.ssl_certificate_expires_at
                ? new Date(row.ssl_certificate_expires_at)
                : null,
            status: row.status,
...
//# sourceMappingURL=siteManagementService.js.map