...
class AlertService {
    ...
    async getUserAlerts(userId) {
        const result = await this.pool.query(`SELECT id, user_id, name, description, metric_type, condition, threshold,
    ...
    async createAlert(data) {
        const result = await this.pool.query(`INSERT INTO alert_configs (
    ...
    async updateAlert(id, userId, data) {
        ...
        const result = await this.pool.query(`UPDATE alert_configs
       SET ${fields.join(', ')}
       WHERE id = $${paramCount++} AND user_id = $${paramCount++}
       RETURNING id, user_id, name, description, metric_type, condition, threshold,
                 percentage_change, sensitivity, frequency, channels, severity, enabled,
                 recipients, filters, created_by, created_at, updated_at, last_triggered`, values);
        return result.rows[0] || null;
    }
    /**
    ...
    async deleteAlert(id, userId) {
        const result = await this.pool.query(`DELETE FROM alert_configs WHERE id = $1 AND user_id = $2`, [id, userId]);
        return result.rowCount ? result.rowCount > 0 : false;
    }
    /**
     * Toggle alert enabled status
     */
    async toggleAlert(id, userId) {
        const result = await this.pool.query(`UPDATE alert_configs
       SET enabled = NOT enabled, updated_at = NOW()
       WHERE id = $1 AND user_id = $2
       RETURNING id, user_id, name, description, metric_type, condition, threshold,
                 percentage_change, sensitivity, frequency, channels, severity, enabled,
                 recipients, filters, created_by, created_at, updated_at, last_triggered`, [id, userId]);
        return result.rows[0] || null;
    }
    /**
    ...
    async testAlert(data) {
        ...
        const currentValue = await this.getCurrentMetricValue(data.metric_type, data.filters || {});
     ...
     */
    async getCurrentMetricValue(metricType, _filters) {
        let query = '';
        const values = [];
        switch (metricType) {
            case 'donations':
    ...
    async acknowledgeAlert(id, userId) {
        const result = await this.pool.query(`UPDATE alert_instances
       SET acknowledged_by = $1, acknowledged_at = NOW()
       WHERE id = $2
       RETURNING id, alert_config_id, alert_name, metric_type, condition, severity,
                 status, triggered_at, resolved_at, current_value, threshold_value,
                 message, details, acknowledged_by, acknowledged_at`, [userId, id]);
        return result.rows[0] || null;
    }
    /**
    ...
    async resolveAlert(id) {
       ...
       RETURNING id, alert_config_id, alert_name, metric_type, condition, severity,
                 status, triggered_at, resolved_at, current_value, threshold_value,
                 message, details, acknowledged_by, acknowledged_at`, [id]);
        return result.rows[0] || null;
    }
    /**
    ...
    async getAlertStats(userId) {
        // Total alerts
        const totalResult = await this.pool.query(`SELECT COUNT(*) as total FROM alert_configs WHERE user_id = $1`, [userId]);
        // Active alerts
        const activeResult = await this.pool.query(`SELECT COUNT(*) as active FROM alert_configs WHERE user_id = $1 AND enabled = true`, [userId]);
        // Triggered today
        const todayResult = await this.pool.query(`SELECT COUNT(*) as triggered_today
       FROM alert_instances ai
       JOIN alert_configs ac ON ai.alert_config_id = ac.id
       WHERE ac.user_id = $1 AND ai.triggered_at >= CURRENT_DATE`, [userId]);
        // Triggered this week
        const weekResult = await this.pool.query(`SELECT COUNT(*) as triggered_week
       FROM alert_instances ai
       JOIN alert_configs ac ON ai.alert_config_id = ac.id
       WHERE ac.user_id = $1 AND ai.triggered_at >= CURRENT_DATE - INTERVAL '7 days'`, [userId]);
        // Triggered this month
        const monthResult = await this.pool.query(`SELECT COUNT(*) as triggered_month
       FROM alert_instances ai
       JOIN alert_configs ac ON ai.alert_config_id = ac.id
       WHERE ac.user_id = $1 AND ai.triggered_at >= CURRENT_DATE - INTERVAL '30 days'`, [userId]);
        // By severity
        const severityResult = await this.pool.query(`SELECT severity, COUNT(*) as count
       FROM alert_instances ai
       JOIN alert_configs ac ON ai.alert_config_id = ac.id
       WHERE ac.user_id = $1 AND ai.triggered_at >= CURRENT_DATE - INTERVAL '30 days'
       GROUP BY severity`, [userId]);
        const bySeverity = {
            low: 0,
            medium: 0,
            high: 0,
        ...
        // By metric
        const metricResult = await this.pool.query(`SELECT metric_type, COUNT(*) as count
       FROM alert_instances ai
       JOIN alert_configs ac ON ai.alert_config_id = ac.id
       WHERE ac.user_id = $1 AND ai.triggered_at >= CURRENT_DATE - INTERVAL '30 days'
       GROUP BY metric_type`, [userId]);
        const byMetric = {};
