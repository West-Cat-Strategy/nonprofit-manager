...
exports.deleteCase = exports.getCaseStatuses = exports.getCaseTypes = exports.getCaseSummary = exports.createCaseNote = exports.getCaseNotes = exports.updateCaseStatus = exports.updateCase = exports.getCaseById = exports.getCases = exports.createCase = exports.CaseService = void 0;
...
class CaseService {
    ...
    async createCase(data, userId) {
        ...
        const statusResult = await this.pool.query(`SELECT id FROM case_statuses WHERE status_type = 'intake' AND is_active = true ORDER BY sort_order LIMIT 1`);
    ...
    `, [
        ...
        // Create initial note
        await this.pool.query(`INSERT INTO case_notes (case_id, note_type, content, created_by) VALUES ($1, 'note', $2, $3)`, [result.rows[0].id, 'Case created', userId]);
        logger_1.logger.info(`Case created: ${caseNumber}`, { caseId: result.rows[0].id });
    ...
    async getCases(filter = {}) {
        ...
        if (filter.search) {
            ...
            filters.push(`(c.case_number ILIKE $${params.length - 2} OR c.title ILIKE $${params.length - 1} OR c.description ILIKE $${params.length})`);
        }
     ...
     * Update case
     */
    async updateCase(caseId, data, userId) {
        const fields = [];
        const values = [];
        let paramIndex = 1;
        ...
        values.push(caseId);
        const result = await this.pool.query(`UPDATE cases SET ${fields.join(', ')} WHERE id = $${paramIndex} RETURNING *`, values);
        return result.rows[0];
    }
    /**
     * Update case status
     */
    async updateCaseStatus(caseId, data, userId) {
        // Get current status
        ...
        // Update case status
        const result = await this.pool.query(`UPDATE cases SET status_id = $1, modified_by = $2, updated_at = NOW() WHERE id = $3 RETURNING *`, [data.new_status_id, userId, caseId]);
    ...
    `, [caseId, data.notes || 'Status updated', previousStatusId, data.new_status_id, userId]);
        logger_1.logger.info(`Case status updated`, { caseId, newStatus: data.new_status_id });
        return result.rows[0];
    }
    /**
     * Get case notes
     ...
     * Create case note
     */
    async createCaseNote(data, userId) {
        const result = await this.pool.query(`
      INSERT INTO case_notes (
     ...
     */
    async getCaseTypes() {
        const result = await this.pool.query(`SELECT * FROM case_types WHERE is_active = true ORDER BY name`);
        return result.rows;
    }
    /**
     * Get case statuses
    ...
    async getCaseStatuses() {
        const result = await this.pool.query(`SELECT * FROM case_statuses WHERE is_active = true ORDER BY sort_order`);
        return result.rows;
    }
    /**
     * Delete case
    ...
    async deleteCase(caseId) {
        ...
        logger_1.logger.info(`Case deleted`, { caseId });
    }
}
exports.CaseService = CaseService;
// Backwards compatible exports for existing code
const caseServiceInstance = new CaseService(database_1.default);
exports.createCase = caseServiceInstance.createCase.bind(caseServiceInstance);
exports.getCases = caseServiceInstance.getCases.bind(caseServiceInstance);
exports.getCaseById = caseServiceInstance.getCaseById.bind(caseServiceInstance);
exports.updateCase = caseServiceInstance.updateCase.bind(caseServiceInstance);
exports.updateCaseStatus = caseServiceInstance.updateCaseStatus.bind(caseServiceInstance);
exports.getCaseNotes = caseServiceInstance.getCaseNotes.bind(caseServiceInstance);
exports.createCaseNote = caseServiceInstance.createCaseNote.bind(caseServiceInstance);
exports.getCaseSummary = caseServiceInstance.getCaseSummary.bind(caseServiceInstance);
exports.getCaseTypes = caseServiceInstance.getCaseTypes.bind(caseServiceInstance);
exports.getCaseStatuses = caseServiceInstance.getCaseStatuses.bind(caseServiceInstance);
exports.deleteCase = caseServiceInstance.deleteCase.bind(caseServiceInstance);
