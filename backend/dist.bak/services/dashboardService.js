...
class DashboardService {
    ...
    async getUserDashboards(userId) {
        const result = await this.pool.query(`SELECT id, user_id, name, is_default, widgets, layout, breakpoints, cols, created_at, updated_at
    ...
    async createDashboard(data) {
        ...
        try {
            ...
            if (data.is_default) {
                await client.query(`UPDATE dashboard_configs SET is_default = false WHERE user_id = $1`, [data.user_id]);
            }
            const result = await client.query(`INSERT INTO dashboard_configs (user_id, name, is_default, widgets, layout, breakpoints, cols, created_at, updated_at)
     ...
     */
    async updateDashboard(id, userId, data) {
        const client = await this.pool.connect();
        try {
            await client.query('BEGIN');
            // If setting as default, unset other defaults
            if (data.is_default) {
                await client.query(`UPDATE dashboard_configs SET is_default = false WHERE user_id = $1 AND id != $2`, [userId, id]);
            }
            const fields = [];
            const values = [];
            let paramCount = 1;
            ...
            if (fields.length === 1) {
                ...
                const result = await client.query(`SELECT id, user_id, name, is_default, widgets, layout, breakpoints, cols, created_at, updated_at
           FROM dashboard_configs
           WHERE id = $1 AND user_id = $2`, [id, userId]);
                await client.query('COMMIT');
                return result.rows[0] || null;
            ...
            const result = await client.query(`UPDATE dashboard_configs
         SET ${fields.join(', ')}
         WHERE id = $${paramCount++} AND user_id = $${paramCount++}
         RETURNING id, user_id, name, is_default, widgets, layout, breakpoints, cols, created_at, updated_at`, values);
            await client.query('COMMIT');
            return result.rows[0] || null;
        }
        catch (error) {
     ...
     */
    async updateDashboardLayout(id, userId, layout) {
        const result = await this.pool.query(`UPDATE dashboard_configs
       SET layout = $1, updated_at = NOW()
       WHERE id = $2 AND user_id = $3
       RETURNING id, user_id, name, is_default, widgets, layout, breakpoints, cols, created_at, updated_at`, [JSON.stringify(layout), id, userId]);
        return result.rows[0] || null;
    }
    /**
    ...
    async deleteDashboard(id, userId) {
        const result = await this.pool.query(`DELETE FROM dashboard_configs WHERE id = $1 AND user_id = $2`, [id, userId]);
    ...
    async createDefaultDashboard(userId) {
        ...
        const insertResult = await this.pool.query(`INSERT INTO dashboard_configs (user_id, name, is_default, widgets, layout, breakpoints, cols, created_at, updated_at)
