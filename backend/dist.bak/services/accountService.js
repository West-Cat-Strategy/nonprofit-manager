...
class AccountService {
     ...
     */
    async generateAccountNumber(db = this.pool) {
        const result = await db.query(`SELECT account_number
       FROM accounts
     ...
     */
    async getAccounts(filters = {}, pagination = {}, scope) {
        try {
    ...
    async getAccountByIdWithScope(accountId, scope) {
        try {
          ...
          created_by,
          modified_by
        FROM accounts WHERE ${conditions.join(' AND ')}`, values);
            return result.rows[0] || null;
        }
        catch (error) {
            logger_1.logger.error('Error getting account by ID with scope:', error);
            throw new Error('Failed to retrieve account');
        }
    }
    /**
    ...
    async createAccount(data, userId) {
        ...
        try {
            ...
            await client.query('SELECT pg_advisory_xact_lock($1)', [911000001]);
         ...
         RETURNING id as account_id, account_number, account_name, account_type,
           ...
           is_active, created_at, updated_at, created_by, modified_by`, [
            ...
            logger_1.logger.info(`Account created: ${result.rows[0].account_id}`);
    ...
    async updateAccount(accountId, data, userId) {
      ...
      `;
            ...
            logger_1.logger.info(`Account updated: ${accountId}`);
            return result.rows[0];
        }
        catch (error) {
            logger_1.logger.error('Error updating account:', error);
    ...
    async deleteAccount(accountId, userId) {
        try {
            const result = await this.pool.query(`UPDATE accounts 
         ...
         RETURNING id`, [userId, accountId]);
            ...
            logger_1.logger.info(`Account soft deleted: ${accountId}`);
            return true;
        }
        catch (error) {
    ...
    async getAccountContacts(accountId) {
        try {
         ...
         ORDER BY created_at DESC`, [accountId]);
            ...
            logger_1.logger.error('Error getting account contacts:', error);
