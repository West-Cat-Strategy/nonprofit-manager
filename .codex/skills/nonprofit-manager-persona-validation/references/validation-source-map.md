# Validation Source Map

Use this file to choose evidence sources for persona validation work.

## Current Persona Canon

- `../nonprofit-manager-persona-analysis/references/persona-schema.md`
- `../nonprofit-manager-persona-analysis/references/governance-and-revenue-personas.md`
- `../nonprofit-manager-persona-analysis/references/service-delivery-personas.md`
- `../nonprofit-manager-persona-analysis/references/workflow-models.md`

## Validation Surfaces

Use these together when you want the current repo proof chain in order from contract to smoke test to end-to-end runtime:

- `frontend/src/test/ux/personaWorkflowMatrix.ts` for the canonical persona-to-route/workflow map.
- `frontend/src/test/ux/PersonaRouteUxSmoke.test.tsx` for the first-touch route UX smoke and console-error check.
- `e2e/tests/persona-workflows.spec.ts` for seeded end-to-end persona workflow proof.

## Current Summary Docs

- `docs/product/user-personas.md`
- `docs/product/persona-workflows.md`
- `docs/product/README.md`
- `docs/product/product-spec.md`

## Route, Permission, And Feature Sources

- `docs/features/FEATURE_MATRIX.md`
- `docs/features/REPORTING_GUIDE.md`
- `docs/features/CASE_MANAGEMENT_SYSTEM.md`
- `docs/features/FOLLOW_UP_LIFECYCLE.md`
- `docs/features/CASE_CLIENT_VISIBILITY_AND_FILES.md`
- `frontend/src/routes/routeCatalog/**`
- `frontend/src/routes/**`
- `backend/src/utils/permissions.ts`
- `backend/src/utils/roleSlug.ts`
- `backend/src/modules/admin/usecases/roleCatalogUseCase.ts`

## Validation And Historical Inputs

- `docs/validation/README.md`
- `docs/validation/archive/README.md`
- `docs/validation/archive/persona-workflow-audit-2026-04-18.md`
- `docs/validation/archive/executive-director-persona-findings-2026-04-20.md`
- `docs/validation/archive/executive-director-persona-findings-2026-04-20-remediation.md`

## Check Planning

- `../nonprofit-manager-verification-ops/SKILL.md`
- `../nonprofit-manager-verification-ops/references/check-selection.md`
- `../nonprofit-manager-verification-ops/references/runtime-matrix.md`

## External Inputs

Use external benchmark material only after the repo conclusion is already clear:

- `../nonprofit-manager-benchmark-analysis/references/cohort-and-sources.md`
- `../nonprofit-manager-benchmark-analysis/references/persona-pattern-matrix.md`
