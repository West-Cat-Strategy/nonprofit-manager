# Documentation

This directory contains all documentation for the Nonprofit Manager project, organized by category.

**Current Phase:** Phase 2 - Validation & Authorization (In Progress)  
**Last Updated:** February 14, 2026

## Categories

### [API](api/)
- API references, OpenAPI specifications, and integration guides
- [API Integration Guide](api/API_INTEGRATION_GUIDE.md)
- [OpenAPI Specification](api/openapi.yaml)
- [Postman Collections](api/postman/)

### [Backend](backend/)
- Backend development and refactoring guides
- [Backend Service Refactoring Guide](backend/BACKEND_SERVICE_REFACTORING_GUIDE.md)

### [Deployment](deployment/)
- Deployment, database setup, and infrastructure guides
- [Deployment Guide](deployment/DEPLOYMENT.md)
- [Database Setup](deployment/DB_SETUP.md)
- [Plausible Analytics Setup](deployment/PLAUSIBLE_SETUP.md)

### [Development](development/)
- Development conventions, architecture decisions, and guidelines
- [Agent Instructions](development/AGENT_INSTRUCTIONS.md)
- [Code Conventions](development/CONVENTIONS.md)
- [Architecture Decisions](development/ARCHITECTURE.md)
- [Troubleshooting](development/TROUBLESHOOTING.md)
- [Release Checklist](development/RELEASE_CHECKLIST.md)

### [Features](features/)
- Feature-specific documentation and implementation details
- [Case Management System](features/CASE_MANAGEMENT_SYSTEM.md)
- [Dashboard Customization](features/DASHBOARD_CUSTOMIZATION.md)
- [Template System](features/TEMPLATE_SYSTEM.md)
- [People Module](features/PEOPLE_MODULE_ENHANCEMENTS.md)
- [Volunteer Components](features/VOLUNTEER_COMPONENTS_STATUS.md)

### [Performance](performance/)
- Performance optimization guides and reports
- [Performance Optimization Guide](performance/PERFORMANCE_OPTIMIZATION.md)
- [Performance Optimization Report](performance/PERFORMANCE_OPTIMIZATION_REPORT.md)

### [Phases](phases/)
- Project phase completion summaries and planning
- [Planning and Progress](phases/planning-and-progress.md)
- [Product Specification](phases/product-spec.md)
- Phase completion summaries (Phase 1, 2, etc.)

### [Product](product/)
- Product specifications and research
- [Product Specification](product/product-spec.md)
- [Product Analytics Research](product/PRODUCT_ANALYTICS_RESEARCH.md)
- [Stripe Testing Guide](product/STRIPE_TESTING_GUIDE.md)

### [Quick Reference](quick-reference/)
- Quick reference guides for common tasks
- [General Quick Reference](quick-reference/QUICK_REFERENCE.md)
- [Phase 1 Quick Reference](quick-reference/PHASE_1_QUICK_REFERENCE.md)
- [People Module Quick Reference](quick-reference/PEOPLE_MODULE_QUICK_REFERENCE.md)

### [Security](security/)
- Security audits, monitoring, and incident response
- [Security Audit](security/SECURITY_AUDIT.md)
- [Security Monitoring Guide](security/SECURITY_MONITORING_GUIDE.md)
- [Incident Response Runbook](security/INCIDENT_RESPONSE_RUNBOOK.md)

### [Testing](testing/)
- Testing guides, procedures, and status
- [Testing Guide](testing/TESTING.md)
- [Integration Testing Guide](testing/INTEGRATION_TEST_GUIDE.md)
- [Manual Testing Guide](testing/MANUAL_TESTING_GUIDE.md)

### [UI](ui/)
- UI/UX design, theming, and templates
- [Theme Implementation Plans](ui/THEME_IMPLEMENTATION_PLANS.md)
- [UI Template Information](ui/user_interface_template_information.md)

### [Validation](validation/)
- Validation schemas and references
- [Validation Schemas Reference](validation/VALIDATION_SCHEMAS_REFERENCE.md)

## Getting Started

- Start with the main [README](../README.md) for project overview
- Check [Development Conventions](development/CONVENTIONS.md) for coding standards
- Review [Architecture Decisions](development/ARCHITECTURE.md) for design choices
- See [Deployment Guide](deployment/DEPLOYMENT.md) for setup instructions</content>
<parameter name="filePath">/Users/bryan/projects/nonprofit-manager/docs/README.md