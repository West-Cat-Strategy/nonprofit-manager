...
The landed runtime baseline is intentionally small: saved audiences and campaign runs ride with `P5-T3`, donor-profile receipt defaults ride with the receipt-defaulting seam, and typed appeals, fund restrictions, donation batches, memberships, broader campaign ROI, and finance breadth remain queued behind separate scoped rows.
...
The final reference-repo pass adds future refinements only. These do not change the landed `P5-T3` runtime baseline, and they should be opened only as separately scoped rows after the saved-audience, campaign-run, and donor-default seams settle.
...
- Communication fatigue rules, channel-specific DNC reason ledgers, pledge schedules, soft-credit attribution, in-kind eligibility, and public campaign presentation remain queued refinements; none may expand the landed `P5-T3` implementation boundary.
