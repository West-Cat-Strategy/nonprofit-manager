...
**Last Updated:** March 19, 2026 (`P4-T25` slimmed the backend Docker image by switching to Docker-only TypeScript emit; the tagged backend image shrank from 409MB to 399MB, the shipped `dist/` tree is JS-only, and `make docker-validate` plus `make docker-build` passed; `P4-T24` removed the portal browse block from the header and the targeted PortalLayout test passed; `P4-T23` removed the staff workspace browse block from the header and the targeted WorkspaceHeader test passed; `P4-T1R8E` earlier landed the route-catalog/admin redirect simplification: dead alias tables were removed, canonical admin routes now own navigation, the targeted frontend route/nav coverage passed, and `cd frontend && npm run build` passed; repo-wide `make lint` and `make typecheck` still stop on unrelated backend debt.)
...
| P2-T18 | Outcomes tracking for case interactions (definitions, tagging, reports) | Phase 2 | Review | Codex | Mar 14, 2026 | TBD | main (case-linked contact-note outcome tagging, synced case outcomes, interaction-report parity; targeted backend/frontend tests plus frontend/backend type-check passed Mar 14, 2026) |
...
| P4-T18A | CRA tax receipts + shared organization settings for donations | Phase 4 | Review | Codex | Mar 15, 2026 | TBD | main (parent: P4-T18; shared admin organization settings, CRA-style donation receipt issuance/download/email flows, backend receipt persistence, and targeted finance/admin regression coverage landed. Targeted backend Jest + frontend Vitest + frontend type-check passed Mar 15, 2026; repo-wide lint/type-check remain blocked by unrelated pre-existing issues in `backend/src/services/teamChatRealtimeService.ts` and `backend/src/services/publishing/siteOperationsService.ts`.) |
| P4-T19 | Social media tracking module (Facebook-first) | Phase 4 | Review | Codex | Mar 15, 2026 | TBD | main (implementation complete; targeted backend/frontend type-check and social-media route, unit, nav, and page tests passing locally.) |
...
| P4-T21 | Workspace module controls for org admins | Phase 4 | Review | Codex | Mar 15, 2026 | TBD | main (follow-up warning cleanup completed: removed remaining frontend ESLint warnings across runtime code and tests, preserved task search debounce behavior, and reran frontend/repo lint and test verification successfully.) |
| P4-T22 | Unified staff + portal booking calendar for appointments and events | Phase 4 | Review | Codex | Mar 18, 2026 | TBD | main (user-requested implementation lane; unified staff + portal booking calendar landed with visible-range filtering for portal events and appointment slots, a shared booking calendar UI, admin-aware staff appointment/slot actions, and targeted portal regression coverage. Verification passed: `make lint`, `make typecheck`, `cd backend && npm run test:unit`, `cd backend && npm run test:integration`, `node scripts/ui-audit.ts`, and `cd frontend && npm test -- --run`.) |
...
| P4-T26 | Staff help-center screenshot refresh (Workbench Overview copy sync + PNG rebuild) | Phase 4 | Review | Codex | Mar 19, 2026 | TBD | main (docs-only follow-up to P4-T14; aligning the staff help-center copy with the live Workbench Overview labels, refreshing the PNG captures under `docs/help-center/staff/assets/screenshots/`, and leaving the docs-first landing/changing-areas captures untouched. Verification passed: `cd e2e && npm test -- tests/help-center-screenshots.spec.ts`, `make check-links`, `make lint-doc-api-versioning`.) |
| P4-T28 | Cases workflow docs refresh (staff + portal companion guides) | Phase 4 | Review | Codex | Mar 19, 2026 | TBD | main (docs-only follow-up to P4-T26; adding a staff cases workflow article, a portal companion article, staff hub/nav updates, and a cases-only screenshot refresh under staff and portal screenshot trees. Verification passed: `cd e2e && SKIP_WEBSERVER=1 PW_REUSE_EXISTING_SERVER=1 npx playwright test --project=chromium tests/help-center-cases-screenshots.spec.ts`, `make check-links`, and `make lint-doc-api-versioning`.) |
| P4-T27 | Nonprofit Manager LUKS database SSL auto-disable fix | Phase 4 | In Progress | Codex | Mar 19, 2026 | TBD | main (follow-up to P4-T17; backend now auto-disables DB SSL for `DB_AT_REST_ENCRYPTION_MODE=luks` so sign-in no longer 500s against local Postgres, with matching config test and deployment/env doc updates.) |
