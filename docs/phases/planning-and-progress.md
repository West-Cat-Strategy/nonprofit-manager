...
**Live Snapshot:** 26 active rows: 13 In Progress, 2 Blocked, 11 Review, 0 Ready.
...
| Harden deploy containers (019da3c3-8b11) | top-level | Apr 18, 2026 | Blocked | — | Thread was stopped mid-lane; archive export/restore scripts have the unfinished Bash guardrail bug, and the workspace-root `npm ci` Dockerfile path still lacks end-to-end build verification. |
...
|----|------|-------|--------|-------|---------|--------|-----------|
| P4-T47 | Public history scrub and `.gitignore` hardening | Phase 4 | In Progress | Codex | Apr 15, 2026 | TBD | main (active 2026-04-19: user-requested follow-through is reversing the prior in-repo `.codex` tracking decision, broadening `.gitignore` toward a maximum local-only policy for AI/editor workspace artifacts, removing `.codex/**` from the current tip while keeping local files in place, updating contributor docs/workboard artifacts that still describe `.codex/skills` as versioned repo guidance, and preparing an immediate `git-filter-repo` rewrite plus force-push/cache-purge evidence so `origin/main` no longer exposes `.codex` in tip or history) |
