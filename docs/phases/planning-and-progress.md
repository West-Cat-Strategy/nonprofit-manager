...
| P4-T7E-DARK | Whole-app dark-mode accessibility remediation + route-audit closure | Phase 4 | In Progress | Codex | Mar 11, 2026 | TBD | main (in progress 2026-04-19: the full Chromium dark-mode audit rerun is now executed on the integrated tree, and it failed with real route/runtime/contrast defects rather than harness drift. Representative blockers include final-location drift on `/`, `/setup`, `/events`, `/events/calendar`, `/grants`, and `/websites/:siteId`; runtime/API failures on admin-registration review, public event check-in, opportunities, meetings, outreach, and multiple grants routes; and contrast failures on `/cases/:id/edit`, `/people`, and `/settings/admin/branding`. Mobile Safari availability is confirmed, so this row is not environment-blocked. Next step: fix the failing routes and restore the missing `e2e/test-results/dark-mode-accessibility-report.md` artifact emission before attempting closeout again.) |
