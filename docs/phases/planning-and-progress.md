...
**Live Snapshot:** 27 active rows: 6 In Progress, 2 Blocked, 19 Review, 0 Ready.
...
**Recent thread follow-through:** 5 rows: 3 In Progress, 2 Blocked.
...
- Coordinated exception, 2026-04-19: `P4-T9` is split across parallel lanes.
  ...
  Other lanes: `route-trust-docs`, `e2e-contract`, `coverage-proof`
  Integration owner: `Codex`
...
- Coordinated exception, 2026-04-19: `P4-T7` is split across parallel lanes.
  Lead: `Codex`
  Backend lanes: `none`
  Frontend lanes: `framework-shell`, `portal-auth-public`
...
| Review test coverage (019da39f) | top-level | Apr 18, 2026 | In Progress | `P4-T9` | Coverage execution is reopened under the coordinated `coverage-proof` lane. Current goal: restore trustworthy backend/frontend coverage runs, add the planned high-risk coverage tests, and tighten frontend measurement so untouched files count. |
| Implement CI/E2E contract lane (019da34a-d770) | top-level | Apr 18, 2026 | In Progress | `P4-T9` | The shared host-authenticated portal route set and compact-card Mobile Chrome proof are green again after the staff-shell/workbench spacing trims plus the `/events` default-date fallback fix. The coordinated `e2e-contract` lane now narrows to the deferred `Mobile Safari` / `Tablet` / dark-mode reruns and any remaining harness-trust fixes needed to keep the stricter final-location/runtime assertions authoritative. |
...
| Review UI/UX usability (019da3ce) | top-level | Apr 18, 2026 | In Progress | `P4-T7` | The strategic staff-app audit artifact still seeds the coordinated `staff-ux-audit` lane, and the paired framework-shell follow-through is now landed: Workbench naming is consistent again, staff local navigation is restored on mobile, and the compact-shell Mobile Chrome regression proof is green. Remaining work is the authenticated walkthrough synthesis/backlog update, not more shell plumbing. |
...
| P4-T7E-DARK | Whole-app dark-mode accessibility remediation + route-audit closure | Phase 4 | In Progress | Codex | Mar 11, 2026 | TBD | main (in progress 2026-04-19: this row is signed out alongside the ready `P4-T9` `e2e-contract` follow-through now that the authenticated portal/dashboard route blockers are cleared and the dark-mode audit list run is trustworthy again. Lead owns the shared audit artifact and workboard closeout; next step: run the full findings-producing audit and publish the refreshed `e2e/test-results/dark-mode-accessibility-report.md` artifact.) |
...
| P4-T1R4 | Wave 2+ rollout prep (reports/analytics/dashboard then ops/comms surfaces) | Phase 4 | In Progress | Codex | Mar 2, 2026 | TBD | main (in progress 2026-04-19: coordinated exception remains active with lead-owned workboard/shared seams plus `admin-analytics-contracts`, `admin-analytics-ui`, and `reports-admin-browser-proof` lanes. The backend `admin-analytics-contracts` lane is now landed: admin branding, alerts, and analytics integration coverage assert the canonical response envelopes directly, and `cd backend && npm test -- src/__tests__/integration/adminBranding.test.ts src/__tests__/integration/alerts.test.ts src/__tests__/integration/analytics.test.ts` passed. The remaining work is the paired UI/browser proof follow-through.) |
...
| P4-T7 | Full app UI/UX replacement (all themes, all routes) | Phase 4 | In Progress | Codex | Apr 16, 2026 | TBD | main (in progress 2026-04-19: coordinated exception stays active with lead-owned workboard/route-catalog seams plus `framework-shell`, `portal-auth-public`, and `staff-ux-audit` lanes. The framework-shell lane is now landed: staff dashboard/home labels consistently read Workbench again, mobile staff local navigation is restored without reintroducing duplicate primary actions, and the compact-shell Mobile Chrome proof is green. Remaining work is the broader portal/auth/public cleanup plus the live staff audit/backlog follow-through captured in [../validation/staff-app-ui-ux-strategic-audit-2026-04-18.md](../validation/staff-app-ui-ux-strategic-audit-2026-04-18.md).) |
...
| P4-T9 | Setup and launch stabilization + test expansion | Phase 4 | In Progress | Codex | Apr 14, 2026 | TBD | main (in progress 2026-04-19: the coordinated `portal-signup-admin`, `portal-runtime`, `route-trust-docs`, and reopened `portal-admin-size` follow-through are landed, and the reopened `e2e-contract` lane has now cleared the deferred host Mobile Chrome compact-card proof after the shared shell/workbench and `/events` fallback fixes. `backend/src/modules/portalAdmin/controllers/portalAdminController.ts` remains a thin export seam over focused shared/account/conversation/appointment controller files, `make quality-baseline` now clears the implementation-size policy for this surface, and the remaining work is the rest of the audit/coverage execution rather than more controller-size cleanup.) |
...
| P4-T48 | Case-centric form builder (case defaults, portal/public completion, client-file recording) | Phase 4 | In Progress | Codex | Apr 16, 2026 | TBD | main (in progress 2026-04-19: coordinated exception stays active with `cases-form-defaults` and `cases-form-completion` lanes. The frontend completion lane is now landed: portal/public case-form flows keep `submitted` forms editable until staff review finishes, receipt messaging/downloads still appear immediately, and the row-local portal/public tests passed. The remaining follow-through is the backend case-default semantics plus any client-file recording work that depends on that default-instantiation pass.) |
