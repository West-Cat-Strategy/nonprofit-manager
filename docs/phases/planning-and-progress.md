...
**Last Updated:** March 15, 2026 (`P4-T1R7` advanced on `main` again with E2E auth/bootstrap hardening for the cases/contacts/outcomes wave: shared auth-state reuse now clears stale bootstrap state before re-login, `loginViaAPI` syncs the cookie-backed session before bootstrap validation, and non-strict setup-complete runs elevate the cached shared user directly instead of repeating the invalid `admin@example.com` path. Verification passed for `make typecheck` plus `cd e2e && CI=1 E2E_USE_DEV_RUNTIME=1 npx playwright test --project=chromium tests/admin.spec.ts tests/accounts.spec.ts tests/cases.spec.ts tests/contacts.spec.ts --retries=0` (`41 passed`). Strict compiled-runtime/full-suite closure is still blocked outside this lane by the in-flight `P4-T18` frontend build regression in `RecurringDonationCheckoutResultPage.tsx`.)
