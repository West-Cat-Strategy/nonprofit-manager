...
| P4-T7E-DARK | Whole-app dark-mode accessibility remediation + route-audit closure | Phase 4 | In Progress | Codex | Mar 11, 2026 | TBD | main (in progress 2026-04-20: the coordinated follow-through is still active, but the remaining surface is much narrower. The integrated passes landed the admin-registration-review backend fix and regression test, the volunteer fixture DB fallback, meeting-detail/edit fixture provisioning, the public case-form fallback helper, the `/api/v2/templates/:templateId/preview` builder-preview URL correction, the no-auto-rewrite `/events` workspace behavior, dark-mode contrast improvements for admin branding, case back buttons, people-directory/demo people, and brighter brutalist card text/focus states, plus the demo route data/runtime cleanup. `cd frontend && npm test -- --run src/features/events/pages/__tests__/EventsHubPage.test.tsx src/features/events/pages/__tests__/EventCalendarPage.test.tsx src/features/events/pages/__tests__/PublicEventCheckInPage.test.tsx src/features/grants/pages/__tests__/GrantsPage.test.tsx src/features/meetings/api/meetingsApiClient.test.ts src/features/neoBrutalist/pages/__tests__/OutreachCenterPage.test.tsx src/features/adminOps/pages/adminSettings/sections/__tests__/BrandingSection.test.tsx src/features/engagement/opportunities/pages/__tests__/OpportunitiesPage.test.tsx src/features/neoBrutalist/pages/__tests__/PeopleDirectoryPage.test.tsx src/components/neo-brutalist/__tests__/PeopleCard.test.tsx src/features/builder/pages/__tests__/TemplatePreviewPage.test.tsx`, `cd frontend && npm run type-check`, `cd backend && DB_HOST=127.0.0.1 DB_PORT=8012 DB_NAME=nonprofit_manager_test DB_USER=postgres DB_PASSWORD=postgres npx jest --runInBand src/__tests__/integration/adminRegistrationReview.test.ts src/modules/admin/__tests__/repositories/pendingRegistrationRepository.test.ts`, `node scripts/check-auth-guard-policy.ts`, `cd e2e && bash ../scripts/e2e-playwright.sh host ./node_modules/.bin/playwright test tests/dark-mode-accessibility-audit.spec.ts --project=chromium --list`, multiple full Chromium reruns, and `git diff --check` passed where applicable. The latest full Chromium rerun now audits all 152 cataloged routes with 154 findings (127 critical, 27 moderate, 0 blocked); `admin-registration-review`, `case-detail`, `case-edit`, and `demo-people` are cleared, and the remaining unresolved work is concentrated in three areas: `public-case-form` still 404s on `GET /api/v2/public/case-forms/:token`, `opportunities` still reports low-contrast workflow cards, and `website-builder-preview` still emits repeated sandboxed `localStorage` runtime errors.) |
...
| P4-T7 | Full app UI/UX replacement (all themes, all routes) | Phase 4 | In Progress | Codex | Apr 16, 2026 | TBD | main (in progress 2026-04-19: coordinated exception stays active with lead-owned workboard/route-catalog seams plus the landed `framework-shell`, `portal-auth-public`, and `staff-ux-core` passes. Default staff destinations now align across desktop/mobile, Workbench is queue-first again, portal/auth/public entry screens keep mobile forms and navigation above the fold, and the targeted frontend suites plus the Mobile Safari / Tablet compact-route proof are green. Remaining work is the dedicated `P4-T7E-DARK` route/accessibility remediation and any follow-through from the updated staff UX audit, not more shell or portal-entry plumbing.) |
...
| P4-T54 | Persona workflow remediation wave (MFA-aware onboarding proof + reports kickoff + fundraiser stewardship kickoff) | Phase 4 | Blocked | Codex | Apr 19, 2026 | TBD | main (blocked 2026-04-19: the persona reports/fundraiser kickoff code is landed, the planned frontend suites plus `cd frontend && npm run type-check` are green, and `cd backend && DB_HOST=127.0.0.1 DB_PORT=8012 DB_NAME=nonprofit_manager_test DB_USER=postgres DB_PASSWORD=postgres npx jest --runInBand src/__tests__/integration/auth.mfa.test.ts src/__tests__/integration/routeGuardrails.test.ts src/modules/users/controllers/__tests__/userController.test.ts src/modules/admin/controllers/__tests__/adminSurfaceControllers.test.ts` passed. The only remaining closeout slice is the fresh-workspace MFA proof on a starter-only external Docker stack with `BYPASS_MFA_FOR_TESTS=false`, but the currently available `nonprofit-dev-*` stack reports `{\"setupRequired\":false,\"userCount\":6}` at `http://127.0.0.1:8004/api/v2/auth/setup-status`, so this row is environment-blocked rather than code-incomplete. Active corrections and backlog boundaries remain in [../validation/executive-director-persona-findings-2026-04-20-remediation.md](../validation/executive-director-persona-findings-2026-04-20-remediation.md).) |
