...
| P4-T49 | Aggressive dead-code prune + compatibility retirement | Phase 4 | Review | Codex | Apr 16, 2026 | TBD | main (review 2026-04-18: second-pass residue cleanup removed duplicated registration controller exports, unused newsletter/Mautic helpers, startup-route and route-catalog dead exports, the orphaned schema export path, and stale daily-security/db-restore script references while preserving intentionally live worker/public-site entrypoints, policy scripts, and compatibility wrappers; `npm run knip`, `cd backend && npm run type-check`, `cd frontend && npm run type-check`, `cd backend && npx jest src/modules/admin/controllers/__tests__/adminSurfaceControllers.test.ts src/__tests__/services/newsletterProviderService.test.ts --runInBand`, and `make check-links` passed) |
