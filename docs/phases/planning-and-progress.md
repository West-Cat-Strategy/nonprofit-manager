...
**Last Updated:** March 8, 2026 (`P4-T1R6B`, `P4-T9C`, and `P4-T9F` advanced to `Review` on `main` after local validation passed: `make lint`, `make typecheck`, `make build`, targeted backend/frontend tests, Caddy validation, and a successful dry-run of `./scripts/verify-cbis.sh` against the live CBIS stack.)  
...
| P4-T9C | Dockerization overhaul (compose hardening, script alignment, docker docs sync) | Phase 4 | Review | Codex | Mar 8, 2026 | TBD | main (coordinated main-release bundle with `P4-T1R6B` + `P4-T9F`; CBIS overlay/health remediation is now locally green after the `routeCatalog` build fix. Verification: `make lint`, `make typecheck`, `make build`, `cd backend && npm test -- --runInBand src/__tests__/integration/healthAliases.test.ts`, `docker run --rm -v \"$PWD/Caddyfile.vps:/etc/caddy/Caddyfile:ro\" caddy:2-alpine caddy validate --config /etc/caddy/Caddyfile`.) |
| P4-T9F | CBIS deploy automation helpers (ref-based snapshot deploy + verification + operator runbook) | Phase 4 | Review | Codex | Mar 8, 2026 | TBD | main (coordinated main-release bundle with `P4-T9C` + `P4-T1R6B`; added committed `deploy-cbis`/`verify-cbis` helpers, CBIS production documentation, and compose env-file plumbing for migration/verification scripts. Verification: `bash -n scripts/deploy-cbis.sh`, `bash -n scripts/verify-cbis.sh`, `./scripts/verify-cbis.sh`.) |
...
| P4-T1R6A | Backend hotspot decomposition (`events`, publishing/site-generation canonicalization) | Phase 4 | Ready | — | — | TBD | — (parent: P4-T1R6) |
| P4-T1R6B | Frontend hotspot decomposition (auth ownership + route metadata/catalog removal + root-store cleanup) | Phase 4 | Review | Codex | Mar 8, 2026 | TBD | main (parent: P4-T1R6; coordinated main-release bundle with `P4-T9C` + `P4-T9F`; admin routing/catalog stabilization is locally green with the route-catalog typing/build fix in place. Verification: `make typecheck`, `make build`, `cd frontend && npm test -- src/routes/__tests__/routeCatalog.test.ts src/routes/__tests__/adminRedirects.test.tsx src/routes/__tests__/setupRedirects.test.tsx src/pages/__tests__/admin/AdminSettings.test.tsx src/features/adminOps/components/__tests__/AdminPanelNav.test.tsx src/components/__tests__/Navigation.test.tsx`.) |
