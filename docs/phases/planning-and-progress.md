...
**Last Updated:** March 18, 2026 (`P4-T1R8` and the active Wave 3 backend adapter rows are closure-ready on `main`: root runtime imports now resolve through feature-owned route/preload surfaces and canonical publishing services, the retained route/publishing wrappers remain documented deprecated facades only, and the activities/webhooks/mailchimp/invitations/meetings controller seams now depend on module-local service adapters while keeping public contracts unchanged. Verification passed for `make lint`, `make typecheck`, `cd backend && npm run test:unit`, `cd backend && npm run test:integration`, `node scripts/ui-audit.ts`, `cd frontend && npm test -- --run`, `node scripts/check-frontend-legacy-page-path-policy.ts`, `node scripts/check-canonical-module-import-policy.ts`, `cd frontend && npm run build`, focused frontend route/nav coverage (`4 files`, `39 tests`), and focused backend publishing/runtime Jest coverage (`3 suites`, `24 tests`).)
