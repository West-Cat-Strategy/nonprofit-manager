...
| P5-T3 | Email platform wave: blast email plus email builder/formatter | In Progress | Codex | Keep `/api/v2/mailchimp/*` as the outbound contract while the reference-repo `borrow now` email slice adds internal saved audiences, run-specific Mailchimp static-segment targeting, campaign-run history, and narrow donor-profile receipt defaults to the existing Mailchimp, communications, and receipting seams. Scheduled campaign creation remains explicit, `POST /api/v2/mailchimp/campaigns` stays the provider draft/schedule contract, and typed appeals, restrictions, donation batches, memberships, and broader campaign ROI remain queued behind separate scoped rows. | [PHASE_5_DEVELOPMENT_PLAN.md](PHASE_5_DEVELOPMENT_PLAN.md) |
| P5-T5 | Client portal wave | In Progress | Codex | The portal forms inbox slice is now green in targeted proof, and case-aware appointments continuity is green in focused Chromium browser proof: `/api/v2/portal/appointments*` remains the canonical contract, `/portal/appointments` keeps selected-case context visible, and appointment cards expose client-safe case and pointperson cues with links back to the case workspace and messages. Keep the next portal pickup sequenced through public-intake resolution (`PAT-04`) with best-effort audit writes, queue view definitions (`PAT-05`) with owner/surface/permission-scoped update limits, then case-scoped typed portal review requests (`PAT-06`) with portal/staff actor attribution, plus object-level authorization, PII, and audit proof before widening again. | [PHASE_5_DEVELOPMENT_PLAN.md](PHASE_5_DEVELOPMENT_PLAN.md) |
