...
| P2-T18 | Outcomes tracking for case interactions (definitions, tagging, reports) | Phase 2 | Done | Codex | Mar 14, 2026 | TBD | main@27b6832 (05dc131 query-only follow-up; case-linked contact-note outcome tagging, synced case outcomes, and interaction-report parity landed on main, with targeted backend/frontend tests plus frontend/backend type-check passing Mar 14, 2026.) |
...
| P4-T26 | Staff help-center screenshot refresh (Workbench Overview copy sync + PNG rebuild) | Phase 4 | Done | Codex | Mar 19, 2026 | TBD | main@fd82d20 (docs-only follow-up to P4-T14; aligned the staff help-center copy with the live Workbench Overview labels, refreshed the PNG captures under `docs/help-center/staff/assets/screenshots/`, and left the docs-first landing/changing-areas captures untouched. Verification passed in the committed lane via `cd e2e && npm test -- tests/help-center-screenshots.spec.ts`, `make check-links`, and `make lint-doc-api-versioning`.) |
...
|------|---------|-------|---------------|-------|
| Mar 19, 2026 | P4-T1R8E | Codex | Review → Done | Closed the repo-wide simplicity sweep follow-up on main@05dc131: dead route-catalog alias removal, canonical admin routing, and compatibility-facade cleanup are already on main; targeted frontend route/nav tests and the frontend build passed in the committed lane. |
...
| Mar 19, 2026 | P4-T26 | Codex | Review → Done | Closed the staff help-center screenshot refresh lane on `main@fd82d20`: Workbench Overview copy now matches the live labels, the PNG refresh landed under `docs/help-center/staff/assets/screenshots/`, and the committed verification trail remains `cd e2e && npm test -- tests/help-center-screenshots.spec.ts`, `make check-links`, and `make lint-doc-api-versioning`. |
| Mar 19, 2026 | P4-T27 | Codex | In Progress → Done | Closed the Nonprofit Manager LUKS DB SSL auto-disable follow-up on `main@fd82d20`: `backend/src/config/database.ts` now forces node-postgres SSL off for `DB_AT_REST_ENCRYPTION_MODE=luks`, the config test covers both managed and LUKS production modes, and production deployment/env docs explain the contract. Verification passed in this session with `cd backend && npm test -- --runInBand src/__tests__/config/database.test.ts src/__tests__/config/productionSecurityConfig.test.ts` and `cd frontend && npm run build`. |
