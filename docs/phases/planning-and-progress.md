...
| Implement CI/E2E contract lane (019da34a-d770) | top-level | Apr 18, 2026 | Blocked | `P4-T9` | CI and selector wiring landed, but live Docker smoke closure is still blocked by the existing Playwright failure in `tests/public-website.spec.ts` (`Public website starter` referral flow). |
...
| P4-T7E-DARK | Whole-app dark-mode accessibility remediation + route-audit closure | Phase 4 | In Progress | Codex | Mar 11, 2026 | TBD | main (active 2026-04-18: route-health now fails on same-origin console/network/runtime errors, legacy redirect expectations come from the admin route manifest instead of a hard-coded drift list, and the dark-mode audit flow now documents the compiled preview runtime used for long-running audits; `cd frontend && npm test -- --run src/components/__tests__/AdminRoute.test.tsx src/routes/__tests__/adminRedirects.test.tsx src/features/neoBrutalist/pages/__tests__/OperationsBoardPage.test.tsx src/features/neoBrutalist/pages/__tests__/OutreachCenterPage.test.tsx src/services/loop/__tests__/demo.test.ts src/services/__tests__/portalUnauthorizedHandler.test.ts src/services/bootstrap/__tests__/portalBootstrap.test.ts src/features/portal/pages/__tests__/PortalAccessPages.test.tsx`, `cd e2e && bash ../scripts/e2e-playwright.sh host ./node_modules/.bin/playwright test tests/link-health.spec.ts --project=chromium --grep "loads /demo/dashboard|loads /demo/operations|loads /demo/outreach"`, and `cd e2e && bash ../scripts/e2e-playwright.sh host ./node_modules/.bin/playwright test tests/link-health.spec.ts --project=chromium --grep "loads /portal/login|loads /portal/signup|loads /portal/accept-invitation/test-token"` are green; the remaining browser failures are now narrowed to authenticated admin/portal provisioning (`auth/setup-status` 500s and strict-admin credential drift) rather than anonymous route noise, and the prior `make check-links` pass still stands as historical evidence but a fresh post-doc-update rerun is pending because the wrapper hung idle on retry) |
...
| P4-T1R4W3F | Wave 3 prep: meetings modularization package (backend module + wrapper + frontend feature + tests/policies) | Phase 4 | In Progress | Antigravity | Apr 19, 2026 | TBD | main (active 2026-04-19: implementing the missing frontend feature package, backend integration wrapper, and row-local test suite to satisfy the modularization contract) |
...
| P4-T9 | Setup and launch stabilization + test expansion | Phase 4 | In Progress | Codex | Apr 14, 2026 | TBD | main (active 2026-04-19: the Docker readiness follow-through now makes `make docker-up-dev` and the isolated smoke gate wait on real HTTP endpoints, and fixes the smoke-wrapper env scoping bug that was still sending Playwright to the default `8004/8005/8006` ports even when the isolated stack was on `18004/18005/18006`; `make test-tooling` passed, `make docker-up-dev` now returns only after `8004/8005/8006` answer, and `make test-e2e-docker-smoke` now clears the public login smoke against the isolated stack. The current blocker-resolution lane is addressing three fresh red paths reproduced on 2026-04-19: the host Playwright wrapper waits on `3001/5173` before the Playwright-managed webServer starts, host-suite reuse immediately exposes BrowserRouter-only `useBlocker` crashes in the shared unsaved-changes guard (`AccountForm` and `UserSettingsPage`), and the API-backed E2E fixture helpers are generating CSRF tokens against cookie-backed sessions while forcing Authorization-only writes, which now yields `403 EBADCSRFTOKEN` on `/api/v2/accounts`, `/api/v2/sites`, and `/api/v2/templates`. Docker-backed public-website smoke is still red on the known authenticated admin/bootstrap fallback (`admin@example.com` credential drift, MFA-required fallback, and registration-disabled shared-user fallback), and the broader follow-through remains authenticated E2E provisioning stability (`auth/setup-status` 500s, strict-admin credential/bootstrap drift, and protected portal route failures on `/portal`, `/portal/profile`, `/portal/messages`, and `/portal/cases`) plus the ongoing backend/frontend red-coverage and `test-coverage-full` CI follow-through) |
