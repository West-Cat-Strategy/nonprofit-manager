...
| P4-T9C | Dockerization overhaul (compose hardening, script alignment, docker docs sync) | Phase 4 | Ready | — | — | TBD | codex/p4-t9c-docker-overhaul (Implemented CI-first docker hardening: removed fixed `container_name` from core compose stacks, added compose project scoping + CI overlay, migrated db scripts to compose service/project contract, aligned Makefile/docker docs. Verification split: `make docker-validate`, `bash -n` script checks, `make lint`, `make typecheck` pass; Docker daemon-dependent commands blocked in this shell.) (queue: #1) |
