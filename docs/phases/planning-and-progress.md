...
**Last Updated:** March 5, 2026 (`P4-T1R5` merged via PR #7 to `main@b546e9f` and moved `Review -> Done`; `P4-T9A` strict re-baseline executed and remains infra-blocked on Docker daemon availability.)  
...
| P4-T9C | Dockerization overhaul (compose hardening, script alignment, docker docs sync) | Phase 4 | Review | Codex | Mar 7, 2026 | TBD | main (follow-up efficiency pass complete: backend/frontend prod+dev now share multi-target Dockerfiles, compose owns Node init/health wiring, Docker build contexts are tightened, and the dev Caddy overlay now uses stock `caddy:2-alpine` with no local image prebuild. Verification: `make docker-validate` ✅, production image build ✅, dev image build ✅, dev+Caddy compose config ✅, `make lint` ✅, `make typecheck` ✅, `node scripts/ui-audit.ts` ✅, `cd backend && npm run test:unit` ✅, `cd frontend && npm test -- --run` ✅. Exact `cd backend && npm run test:integration` is currently blocked by a dirty local mode change on `scripts/db-migrate.sh`; equivalent integration verification passed via CI compose DB bring-up + `bash scripts/db-migrate.sh` + `SKIP_INTEGRATION_DB_PREP=1 npx jest --forceExit --runInBand src/__tests__/integration` ✅.) |
...
| P4-T1R6 | Aggressive full-stack modularization wave (event/publishing/auth/routes/store hotspots first) | Phase 4 | In Progress | Codex | Mar 7, 2026 | TBD | main (parent: P4-T1) (Started directly on the live dirty `main` worktree as a coordinated continuation of the active modularization stream. Current in-worktree slice: moved `eventService` implementation under `backend/src/modules/events/services`, removed the duplicate `backend/src/services/siteGenerator/**` namespace, migrated auth pages/state into `frontend/src/features/auth/**`, and moved admin runtime navigation metadata into a feature-owned manifest. Verification passed: modularity policy scripts, `make lint`, `make typecheck`, `node scripts/ui-audit.ts`, targeted backend event/publishing unit tests, targeted publishing integration, and targeted frontend auth/navigation/setup suites.) |
...
| P4-T1R6B | Frontend hotspot decomposition (auth ownership + route metadata/catalog removal + root-store cleanup) | Phase 4 | Ready | — | — | TBD | — (parent: P4-T1R6) |
