...
**Live Snapshot:** 19 active rows: 2 In Progress, 2 Blocked, 10 Review, 5 Ready.
...
| P4-T7E-DARK | Whole-app dark-mode accessibility remediation + route-audit closure | Phase 4 | Blocked | Codex | Mar 11, 2026 | TBD | main (blocked 2026-04-14: the Docker audit now gets past the old immediate `Invalid credentials` failure after the `P4-T9` auth-fixture work, but `npm run test:docker:audit` still hangs during the route sweep and has not produced a clean row-level verdict yet) |
...
| P4-T9 | Setup and launch stabilization + test expansion | Phase 4 | In Progress | Codex | Apr 14, 2026 | TBD | main (active 2026-04-14 blocker lane for Docker-backed E2E auth-fixture stabilization: runtime-aware admin credential defaults are in place, stale shared-user fallback now has a DB-backed recovery path, and the remaining fallout is a mix of Docker audit hangs plus `authenticatedPage` setup timeouts in the mobile slice) |
