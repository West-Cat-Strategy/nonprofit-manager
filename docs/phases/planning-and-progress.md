...
| P4-T9H | Staff backend efficiency wave (stable contracts, query-path + search/index refactor) | Phase 4 | Blocked | Codex | Mar 13, 2026 | TBD | main (parent: P4-T9) (coordinated parallel exception with active `P4-T9E` + `P4-T1R7`: implementation landed for staff list/search/data-load efficiency across cases/tasks/contacts/accounts/activity feed plus non-breaking frontend request shaping. Targeted backend/frontend tests and typechecks passed. Strict closure is blocked before task-owned failures by pre-existing out-of-scope implementation-size lint violations in `frontend/src/features/adminOps/pages/UserSettingsPage.tsx` and `frontend/src/features/adminOps/pages/adminSettings/sections/PortalSection.tsx`; runtime `EXPLAIN ANALYZE` capture is additionally blocked in this session because Docker is unavailable and no local Postgres server is running.) |
