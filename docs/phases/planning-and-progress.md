...
**Live Snapshot:** 83 active rows: 6 In Progress, 1 Blocked, 70 Review, 6 Ready.
...
- Coordinated exception, 2026-04-14: the in-progress rescue wave is split across parallel frontend, backend/integrations, scripts/skills, and validation tracks; `P4-T37`, `P4-T38`, and `P4-T39A` keep their existing owners, while Codex may land dependency-only unblockers that support their closure.
...
| P4-T9A | Efficiency remediation pack (Top 15 findings) | Phase 4 | Review | Codex | Mar 5, 2026 | TBD | main (implementation-size and frontend bundle policy gates are green on 2026-04-14, and `docs/phases/AUTH_ALIAS_USAGE_REPORT_2026-04-14.md` now records the deferred alias-telemetry status; snake_case alias retirement remains a dated follow-up no earlier than 2026-07-01) |
| P4-T9E | Cross-surface startup + navigation efficiency wave | Phase 4 | Review | Codex | Mar 7, 2026 | TBD | codex/p4-t9e-startup-navigation-wave (Docker-backed Chromium startup guard passed on 2026-04-14 after tightening the request counter to measure backend API traffic only) |
| P4-T18 | Provider-agnostic donation processing via website forms + Stripe/PayPal/Square | Phase 4 | Review | Codex | Mar 15, 2026 | TBD | main (provider-aware donation settings now span backend routes, site settings, public website form handling, website integrations/forms UI, builder defaults, and regression tests; Docker-backed validation is green on 2026-04-14 across the publishing integration slice, the public website render smoke, and the published public donation-form submit endpoint) |
| P4-T29 | Internal grants tracking platform | Phase 4 | Review | Codex | Mar 20, 2026 | TBD | main (former `P4-T35` grants smoke-harness Chromium regression from `e2e/tests/grants.spec.ts:561` cleared on 2026-04-14; frontend grants coverage is green, the backend integration slice now passes with the isolated test database contract, and the Docker-backed grants smoke rerun is green) |
...
| P4-T38 | Contacts module redesign + legacy alignment | Phase 4 | In Progress | Antigravity | Apr 10, 2026 | TBD | main (former `P4-T35` contacts delete-from-list Chromium regression from `e2e/tests/contacts.spec.ts:1082` and former `P4-T9H` implementation-size unblock for `contactMergeService.ts` cleared on 2026-04-14; the rescue-wave contacts mocked-state failures and contacts->cases frontend ownership leakage now pass in targeted slices, so remaining validation tracks task-owned scope only) |
| P4-T39 | Reliability and efficiency remediation sweep | Phase 4 | Review | Codex | Apr 11, 2026 | TBD | main (targeted frontend reliability slices plus Docker-backed backend integration coverage for payments, webhooks, and reconciliation are green on 2026-04-14; the narrow retry/error Chromium slice across `payments.spec.ts`, `webhooks.spec.ts`, and `reconciliation.spec.ts` is also green) |
| P4-T39A | Backend reliability hardening (reconciliation, payments, webhooks) | Phase 4 | Review | Antigravity | Apr 11, 2026 | TBD | main (former `P4-T9H` implementation-size unblock for `webhookService.ts` cleared on 2026-04-14; backend reliability integration files and the Docker-backed retry/error Chromium slice are green, so the remaining workboard row is review-only and stays backend-only) |
| P4-T43 | Contacts/cases paired module split follow-up | Phase 4 | Review | Codex | Apr 13, 2026 | TBD | main (contacts-owned case/reminder adapters now replace the direct contacts->cases state imports in the targeted detail/notes/communications seams, and the related contacts tests plus frontend type-check are green on 2026-04-14) |
...
| P4-T9H | Staff backend efficiency wave (stable contracts, query-path + search/index refactor) | Phase 4 | Blocked | Codex | Mar 15, 2026 | TBD | main (implementation-size closure is unblocked on 2026-04-14 and the previously cited targeted route/messaging/contacts/accounts/cases/events/finance frontend Vitest slices now pass; strict closure stays blocked until the repo-wide `make ci-full` rerun happens after the rescue-wave changes are fully validated in one pass) |
...
| P4-T42 | Repo-local Codex skill suite rebuild | Phase 4 | Review | Codex | Apr 13, 2026 | TBD | main (`.gitignore` now re-includes `.codex/skills/**`, the repo-local skill suite is listed by `git ls-files .codex/skills`, and contributor docs now treat the in-repo skills as versioned workflow guidance) |
