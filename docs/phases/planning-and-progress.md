...
**Last Updated:** March 11, 2026 (`P4-T7E-DARK` signed out on `main` for whole-app dark-mode accessibility remediation driven by the new route audit; `P4-T15` moved to Review after the README-centric development documentation restructure landed across the contributor hubs, service READMEs, testing guides, and documentation governance pages. `P4-T7C-PEOPLE` remains in progress for contacts profile consistency and people-page UX fixes; `P4-T7E-VALID`, `P4-T7E-STAFFQA`, and `P4-T7E-INPUT` remain in Review with their existing verification notes unchanged.)  
...
| P4-T7E-VALID | Strict request-validation hardening + fresh-install SMTP defaults (email normalization, boolean/date parsers, admin email guidance) | Phase 4 | Review | Codex | Mar 11, 2026 | Mar 11, 2026 | main (parent: P4-T7E) (coordinated parallel exception with active `P4-T7C-PEOPLE`: scope stayed isolated to shared/backend validation schemas, admin email-settings UX/tests, and migration/initdb parity; verification passed for `make lint`, `make typecheck`, backend unit/integration, `node scripts/ui-audit.ts`, `make db-verify`, `cd e2e && npm run test:smoke`, focused validation/email-settings tests, and focused frontend Vitest. Broad frontend Vitest still shows an unrelated `RouteUxSmoke`/`RolesSection` failure outside this lane.) |
