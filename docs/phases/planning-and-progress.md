...
**Last Updated:** March 5, 2026 (`P4-T9D` strict-closure unblock completed; task moved `Blocked -> Review -> Done` with updated verification/performance evidence.)  
...
| P4-T1R4W3A | Wave 3 prep: alerts modularization package (backend module + wrapper + frontend feature + tests/policies) | Phase 4 | Ready | — | — | TBD | — (parent: P4-T1R4D) (queue: #4) |
...
| P4-T9A | Efficiency remediation pack (Top 15 findings) | Phase 4 | Blocked | Codex | Mar 4, 2026 | TBD | codex/p4-t9a-startup-efficiency-wave3 (What: Startup efficiency implementation is complete but strict closure gates still fail. Why: External runtime blockers remain (`make ci-full` container-name conflict; strict-admin credential drift in `cd e2e && npm run test:ci`). Next step: Resolve docker naming/auth-seed drift, rerun strict gate pair, then move to Done.) |
...
| P4-T1R5 | Full remaining `/api/v2` modularization sweep (backend-first, contract-stable) | Phase 4 | Review | Codex | Mar 5, 2026 | Mar 5, 2026 | codex/p4-wave-close-direct-cutover (parent: P4-T1) (strict closure sequence now passed end-to-end on Mar 5, 2026 after the final CI stabilization patch in `e2e/tests/performance.startup.spec.ts`; `make ci-full` ✅ and `cd e2e && npm run test:ci` ✅ with retry-resolved flakes only. Modular boundary checks and legacy-surface cleanup evidence remain green.) |
| P4-T1R5A | Backend all-legacy-v2 module cutover (22 remaining domains) | Phase 4 | Review | Codex | Mar 5, 2026 | TBD | codex/p4-wave-close-direct-cutover (parent: P4-T1R5) (completed backend direct cutover for wave-close targets: module controllers now canonical, module-local handlers added, legacy controllers converted to wrappers, task/case follow-up + portal conversation routes re-wired to module-owned handlers, and module policy checks expanded; `make lint`/`make typecheck`/`cd backend && npm run test:unit` passed.) |
| P4-T1R5B | Frontend admin/portal feature ownership cutover (`alerts/webhooks/mailchimp/portalAuth/adminOps`) | Phase 4 | Review | Codex | Mar 5, 2026 | TBD | codex/p4-wave-close-direct-cutover (parent: P4-T1R5) (completed migrated-domain frontend cutover in this branch: feature-owned pages canonicalized for accounts/contacts/volunteers/tasks/cases/analytics/reports/savedReports/scheduledReports/dashboard/followUps, migrated `store/slices/*` wrappers removed, store reducer wiring moved to `features/*/state`, baseline ratcheted to `0`, and frontend Vitest suite passed.) |
| P4-T1R5C | Policy ratchets + modularity cleanup + documentation closure | Phase 4 | Review | Codex | Mar 5, 2026 | Mar 5, 2026 | codex/p4-wave-close-direct-cutover (parent: P4-T1R5) (aggressive simplicity closure complete: scoped legacy page/controller wrappers deleted, tests/docs migrated to feature/module ownership, new legacy reintroduction policy gates active in `Makefile` + `scripts/ci.sh`, and post-metrics/import checks are zero (`legacy_file_count=0`, `legacy_loc_total=0`, `legacy_wrapper_count=0`, source import hits `0`). Strict steps 1-9 now pass, including `make ci-full` and `cd e2e && npm run test:ci`.) |
