...
| P4-T1R4 | Wave 2+ rollout prep (reports/analytics/dashboard then ops/comms surfaces) | Phase 4 | Blocked | Codex | Mar 2, 2026 | TBD | codex/p4-t1r4-wave2-modularization (parent: P4-T1) (What: Wave-2 scope is implemented but closure evidence remains incomplete. Why: Subsequent strict-gate retries were blocked by out-of-scope frontend failures and missing final merge proof. Next step: Re-run scoped strict verification and append merge/closure artifact.) |
| P4-T1R4A | Wave 2 backend modules + dual-stack wrappers (`analytics/reports/saved-reports/scheduled-reports/dashboard/follow-ups`) | Phase 4 | Blocked | Codex | Mar 2, 2026 | TBD | codex/p4-t1r4-wave2-modularization (parent: P4-T1R4) (What: Backend wave-2 subtask implementation exists but closure is not finalized. Why: Final strict gate was blocked by out-of-scope failures after review handoff. Next step: Re-run backend-focused closure matrix and attach artifact.) |
...
| P4-T7C-WEB3 | Website site console + conversion stack (overview/content/forms/integrations/publishing + site-aware builder launch) | Phase 4 | In Progress | Codex | Mar 6, 2026 | TBD | codex/p4-t7i-staff-ux-audit (parent: P4-T7C) (coordinated parallel exception with active `P4-T7I`: scope isolated to website staff workspace, publishing site-ops APIs, site settings merge, and frontend `/websites/*` routes for existing org-scoped sites. Initial pass targets existing-site upgrade path, Mailchimp/Stripe operational settings, form registry surfacing, and site-aware builder launch without changing portal/auth shell ownership.) |
