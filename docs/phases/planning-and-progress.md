...
| P4-T18 | Provider-agnostic donation processing via website forms + Stripe/PayPal/Square | Phase 4 | In Progress | Codex | Mar 15, 2026 | TBD | main (former `P4-T9H` implementation-size unblock for `recurringDonationService.ts` and `publicWebsiteFormService.ts` cleared on 2026-04-14; remaining task validation now tracks task-owned scope only) |
| P4-T29 | Internal grants tracking platform | Phase 4 | In Progress | Codex | Mar 20, 2026 | TBD | main (former `P4-T35` grants smoke-harness Chromium regression from `e2e/tests/grants.spec.ts:561` cleared on 2026-04-14) |
...
| P4-T38 | Contacts module redesign + legacy alignment | Phase 4 | In Progress | Antigravity | Apr 10, 2026 | TBD | main (former `P4-T35` contacts delete-from-list Chromium regression from `e2e/tests/contacts.spec.ts:1082` and former `P4-T9H` implementation-size unblock for `contactMergeService.ts` cleared on 2026-04-14; remaining task validation now tracks task-owned scope only) |
| P4-T39 | Reliability and efficiency remediation sweep | Phase 4 | In Progress | Codex | Apr 11, 2026 | TBD | main |
| P4-T39A | Backend reliability hardening (reconciliation, payments, webhooks) | Phase 4 | In Progress | Antigravity | Apr 11, 2026 | TBD | main (former `P4-T9H` implementation-size unblock for `webhookService.ts` cleared on 2026-04-14; remaining task validation now tracks task-owned scope only) |
...
| P4-T9H | Staff backend efficiency wave (stable contracts, query-path + search/index refactor) | Phase 4 | Blocked | Codex | Mar 15, 2026 | TBD | main (implementation-size closure unblocked on 2026-04-14: `contactMergeService.ts`, `recurringDonationService.ts`, `publicWebsiteFormService.ts`, `siteOperationsService.ts`, `websitesCore.ts`, `webhookService.ts`, `EditorCanvas.tsx`, and `EmailMarketingPage.tsx` now pass `node scripts/check-implementation-size-policy.ts`; focused grants/contacts Chromium slices, backend service checks, `make lint`, and `make typecheck` are green. Strict closure remains blocked by unrelated repo-wide `make ci-full` frontend failures, including `frontend/src/features/alerts/pages/AlertsConfigPage.tsx` import resolution, `frontend/src/routes/__tests__/routeCatalog.test.ts`, `frontend/src/features/messaging/__tests__/drafts.test.ts`, and stale mocked-state failures in contacts/accounts/cases/events/finance UI tests.) |
