...
**Last Updated:** March 8, 2026 (`P4-T1R6B`, `P4-T9C`, and `P4-T9F` moved `Review -> Done` after `main@74db868` was pushed and deployed to `cbis.westcat.ca` via `./scripts/deploy-cbis.sh`, with remote/internal/public verification all passing.)  
...
| P4-T1R4B | Wave 2 frontend feature migration + slice shim cutover (analytics/reporting/dashboard/follow-ups) | Phase 4 | Blocked | Codex | Mar 2, 2026 | TBD | codex/p4-t1r4-wave2-modularization (parent: P4-T1R4) (What: Frontend wave-2 cutover was delivered but closure remains pending. Why: Strict run context contained unrelated failing surfaces and no explicit merged artifact in-row. Next step: Re-run frontend wave-2 closure checks and append artifact.) |
...
| P4-T7B | Global app shell + navigation replacement | Phase 4 | Ready | — | — | TBD | — (parent: P4-T7) (queue: #13) |
...
| P4-T11 | People import/export + reporting export standardization | Phase 4 | In Progress | Codex | Mar 8, 2026 | TBD | main (coordinated parallel exception with active `P4-T1R6` + `P4-T9E`: scope isolated to accounts/contacts/volunteers import/export, ingest commit flow, shared CSV/XLSX export standardization, and associated frontend/reporting download wiring.) |
...
| P4-T1R6 | Aggressive full-stack modularization wave (event/publishing/auth/routes/store hotspots first) | Phase 4 | In Progress | Codex | Mar 7, 2026 | TBD | main (parent: P4-T1) (Started directly on the live dirty `main` worktree as a coordinated continuation of the active modularization stream. Current in-worktree slice: moved `eventService` implementation under `backend/src/modules/events/services`, removed the duplicate `backend/src/services/siteGenerator/**` namespace, migrated auth pages/state into `frontend/src/features/auth/**`, and consolidated admin runtime navigation metadata into the shared route catalog. Verification passed: modularity policy scripts, `make lint`, `make typecheck`, `node scripts/ui-audit.ts`, targeted backend event/publishing unit tests, targeted publishing integration, and targeted frontend auth/navigation/setup suites.) |
| P4-T1R6A | Backend hotspot decomposition (`events`, publishing/site-generation canonicalization) | Phase 4 | Review | Codex | Mar 8, 2026 | Mar 8, 2026 | main (parent: P4-T1R6) (Completed the backend hotspot split on the live dirty `main` worktree: `EventService` is now a thin facade over catalog/registration/public/reminder collaborators, repository/usecase `unknown` returns were tightened to concrete event contracts, and public site runtime rendering moved to module-owned builder/route/context/renderer collaborators with the stable `@services/publishing/publicSiteRuntimeService` surface preserved as a re-export. Verification: targeted backend lint on touched files ✅; `cd backend && npm test -- --runInBand src/__tests__/services/eventService.test.ts src/__tests__/services/publishing/publicSiteRuntimeService.test.ts` ✅ (`41/41`); broader `cd backend && npm test -- --runInBand src/__tests__/integration/events.test.ts src/__tests__/integration/publishing.test.ts` blocked before reaching this scope by unrelated dirty-worktree import gaps in `src/modules/shared/import/peopleImportParser.ts` (`@ingest/preview`, `@ingest/schemaRegistry`, `@ingest/types` missing); `make lint` / `make typecheck` likewise fail on unrelated import/export/import-parser work outside this task.) |
...
| P4-T1R6C | Remaining domain completion (finance/builder/workflows/outcomes + thin backend modules) | Phase 4 | Ready | — | — | TBD | — (parent: P4-T1R6) |
| P4-T1R6D | Policy/docs closure and legacy reintroduction ratchets | Phase 4 | Ready | — | — | TBD | — (parent: P4-T1R6) |
