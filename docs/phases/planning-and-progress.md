...
| Status | ID | Task | Immediate Next Move |
|---|---|---|---|
| Ready | P5-T8 | Codex skill suite audit and refresh | Audit repo-local and global Codex skills against `AGENTS.md`, contributor docs, current runtime/testing guidance, validation references, and the live workboard. Repair stale CBIS importer routing, runtime guidance, broken references, and UX/plugin routing, then decide which helper-skill guidance remains local-only versus tracked canon. |
...
- Pick up `P5-T8` early to refresh Codex skill routing before more tracked work relies on stale helper guidance.
- Keep `P5-T3` in progress only for the narrowed Mailchimp `priorRunSuppressionIds` validation gap, focused route proof, webhook authenticity review, and PII-safe logging follow-through; attach a compact row-local proof note or explicit durable validation link before closeout.
...
- Security hardening remains recorded under `P5-T2B`, not as a fourth product wave: the deliberate backend `exceljs -> uuid` remediation is landed, the live security baseline is green again, the auth-alias operational handoff is published, and the shared Docker CI gate is green. The active email follow-through and portal review slices must keep permission, PII/audit, rate-limit, and SSRF proof current through the published [../validation/PHASE_5_SECURITY_REVIEW_2026-04-22.md](../validation/PHASE_5_SECURITY_REVIEW_2026-04-22.md).
- Product execution now has the portal, website, modularization, volunteer-dispatch, and reassessment runtime pickups in `Review`, while `P5-T3` remains `In Progress` only for the narrowed Mailchimp route-validation and webhook-hardening follow-through found during review. Row-local signoff should focus on proof review and closeout rather than opening new runtime scope inside those rows.
...
- `P5-T5` and `P5-T6C1` are now in `Review`: the portal public-intake/queue/escalation slice and case reassessment cadence slice are landed in targeted proof. `P5-T3` stays `In Progress` until the Mailchimp `priorRunSuppressionIds` route-validation gap, focused route proof, webhook authenticity review, and PII-safe logging follow-through are resolved. `P5-T6` still sits in `Review` as the planning packet feeding `P5-T6A`, `P5-T6B`, and `P5-T6C`, and later typed-domain runtime rows still need separate signout.
...
- Lane: `fundraising-ops-brief`
  Goal: synthesize benchmark-backed fundraising carry-over on saved audiences, campaign-run history, donor preference/receipting, and the boundary into typed appeals; the narrow saved-audience, campaign-run, and donor-profile default pickups are now tied to the active `P5-T3` runtime slice
...
- Lane: `portal-ops-brief`
  Goal: synthesize the next portal carry-over on shared public-intake resolution, reusable queue definitions, shared triage-shell reuse inside `PAT-05`, and typed portal escalations; the narrow `PAT-04`, `PAT-05`, and `PAT-06` pickups are now tied to the active `P5-T5` runtime slice
...
| P5-T8 | Codex skill suite audit and refresh | Ready | Codex | Audit repo-local and global Codex skills against `AGENTS.md`, contributor docs, current runtime/testing guidance, validation references, and the live workboard. Repair stale CBIS importer routing, runtime guidance, broken references, and UX/plugin routing, then decide which helper-skill guidance remains local-only versus tracked canon. | [../../AGENTS.md](../../AGENTS.md), [../testing/TESTING.md](../testing/TESTING.md), [../validation/README.md](../validation/README.md) |
