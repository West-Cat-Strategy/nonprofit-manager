...
Phase 5 is no longer at its kickoff shape. `P5-T1` is complete, `P5-T2A` is the published testing-strategy review artifact, `P5-T2B` has a green final uninterrupted Docker CI artifact, and `P5-T2C`, `P5-T2D`, `P5-T4`, `P5-T5`, `P5-T6`, `P5-T6A`, `P5-T6B`, `P5-T6C`, `P5-T6C1`, `P5-T6D`, `P5-T7`, `P5-T8`, and `P5-T11` are in `Review`. `P5-T3`, `P5-T9`, `P5-T10`, and `P5-T12` remain ready rows for email follow-through, dead-code review, dead-docs review, and the later full E2E/Playwright clean-green validation lane.
...
### 3. Product Wave: Email Platform (Ready Follow-through)
...
- The current `borrow now` runtime pickup is limited to internal saved audiences, run-specific provider static-segment targeting, campaign-run history, and donor-profile receipt defaults. Keep those attached to the live Mailchimp, communications, and tax-receipt seams. `P5-T3` is back in `Ready` until the `priorRunSuppressionIds` route-validation gap, focused route tests, webhook authenticity review, and PII-safe logging follow-through are explicitly signed out again.
...
- Keep `P5-T11` in review against [../refactoring/MODULARITY_SIMPLICITY_REUSE_HANDOFF_2026-04.md](../refactoring/MODULARITY_SIMPLICITY_REUSE_HANDOFF_2026-04.md); use `P5-T9`/`P5-T10` for deeper dead-code and dead-doc follow-through.
...
| `P5-T3` | Blast email and email builder/formatter | Feature plan, implementation slice, validation slice, provider static-segment saved-audience targeting, campaign-run history, donor-profile receipt-default handoff, and narrowed validation/webhook follow-through | `P5-T2B` proof is green; remaining work is row-local Mailchimp route validation and webhook hardening | Row is `Ready` until `priorRunSuppressionIds` route validation, focused route tests, webhook authenticity review, PII-safe logging follow-through, targeted email proof, and `make db-verify` are explicitly signed out again while typed appeals/restrictions stay queued |
...
| `P5-T8` | Codex skill suite audit and refresh | Refreshed repo-local, global, and GitHub-side helper routing, runtime guidance, broken-reference cleanup, and helper-canon decision | Current contributor docs, validation docs, and live workboard | Row is in `Review`; GitHub Copilot/custom-agent guidance remains advisory and must not enable workflows, hooks, MCP config, SaaS review bots, or app-level agents without a separate approval |
...
| `P5-T11` | Comprehensive modularity and simplicity refactor plan | Durable refactor plan, low-risk behavior-preserving cleanup, and handoff evidence across backend, frontend, docs, tests, and shared seams | Current modularization guidance and review-row closeout state | Row is in `Review` against [../refactoring/MODULARITY_SIMPLICITY_REUSE_HANDOFF_2026-04.md](../refactoring/MODULARITY_SIMPLICITY_REUSE_HANDOFF_2026-04.md); deeper cleanup belongs to `P5-T9`/`P5-T10` or a new scoped row |
...
- GitHub Copilot custom agents and path-specific repository instructions as an advisory contributor layer only: route backend, frontend, verification, docs/workboard, and security/integrations lanes back to canonical docs and the existing subagent contract before considering any workflow or third-party bot adoption.
