...
This packet publishes the five capability briefs called for by `P5-T6`. It turns the benchmark and repo-review wave into explicit `borrow now`, `queue for P5-T6`, and `reject` decisions without authorizing runtime implementation beyond the scoped landed pickups tied to `P5-T3`, `P5-T5`, and `P5-T6D`.

The `2026-06-12` stale-doc cleanup keeps the packet analysis-led while acknowledging that the narrow runtime pickups tied to `P5-T3`, `P5-T5`, and `P5-T6D` have landed: saved audiences, campaign-run history, donor-profile receipt defaults, public-intake resolution audit, queue view definitions, portal escalations, and the active task/event picker contract. It does not expand the cohort, authorize typed appeals/restrictions/memberships, or allow direct source copying.
...
- `borrow now`: the capability is adjacent to a real routed seam and should shape the next scoped follow-through after the landed product baseline.
...
- Hold `PAT-07` behind the landed `P5-T3` handoff and use it to shape `P5-T6B`, not the email-wave baseline.
...
- Keep this as the first post-appointments-continuity carry-over from the landed `P5-T5` baseline.
...
- Keep this brief behind the landed `P5-T5` stabilization baseline.
...
- No additional runtime implementation is authorized from this packet until a separate scoped row is signed out on the live workboard. The only landed pickups are the narrow `P5-T3`, `P5-T5`, and `P5-T6D` slices named above; typed appeals, fund restrictions, donation batches, memberships, finance breadth, workflow studios, and metadata builders remain queued or rejected according to their rows.
