...
- Compose files: `docker-compose.yml` + `docker-compose.vps.yml`
...
4. Reloads `nonprofit-manager.service`.
5. Runs `./scripts/db-migrate.sh` with the CBIS compose settings and `.env.production`.
