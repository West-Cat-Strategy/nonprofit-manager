...
| `P5-T94` | Ready follow-up | New residual local-gate cleanup row for unrelated Knip leftovers and UI audit baseline drift found during final lead gates. |
...
- Lead on 2026-05-10: `make lint` failed at UI audit baseline enforcement with `Expected 1517/9933/60, got 1524/9934/60`; implementation-size policy passed before that failure.
- Lead on 2026-05-10: `npm run knip` failed only on unrelated residual findings: `backend/src/modules/mailchimp/controllers/campaignRunActionResponses.ts`, `backend/src/modules/mailchimp/services/mailchimpCampaignRunActionErrors.ts`, and root `express-rate-limit`.
...
- `P5-T94`: resolve residual local-gate failures from unrelated Knip findings and UI audit baseline drift.
...
- `make lint` failed at UI audit baseline enforcement with `Expected 1517/9933/60, got 1524/9934/60`; implementation-size policy passed.
- `make ci-full` remains deferred. The P5-T85 Docker evidence is now reconciled, but the local lint gate is still blocked by `P5-T94`.
