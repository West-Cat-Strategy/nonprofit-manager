...
| P2 | Builder editor authoring affordances | Original audit finding: `frontend/src/features/builder/components/editor/EditorCanvas.tsx` exposed select/delete and add-section actions, while `usePageSectionEditing.ts` exposed add/update/delete/drag handlers only. | Superseded for duplicate/move controls: current editor code exposes duplicate and move controls for components/sections, so this is no longer a hidden live obligation. Keep future authoring refinements, such as keyboard-first placement polish, as separate signed-out rows. | Keep focused component/hook tests around duplicate/delete/move behavior and generated-ID selection continuity. |
...
1. Keyboard-first authoring placement polish if future accessibility review asks for it; duplicate/move controls themselves are no longer a live hidden obligation.
