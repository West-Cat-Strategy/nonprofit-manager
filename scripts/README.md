...
| [verify.sh](verify.sh) | Run the broad repository verification script documented in the verification system note. | Prefer current `make lint`, `make typecheck`, and `make test` unless reproducing that legacy verifier |
| [verify-pr.sh](verify-pr.sh) | Run the PR-specific verification script documented in the verification system note; the current script is still PR-number oriented. | Manual PR verification / historical reproduction |
