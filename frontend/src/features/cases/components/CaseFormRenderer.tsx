              ...
              .map((question) => {
                ...
                return (
                  <div key={question.id} className="space-y-2">
                    <label className={`block text-xs font-black uppercase ${labelClass}`}>
                      {question.label}
                      {question.required ? ' *' : ''}
                    </label>
                    {question.helper_text && (
                      <p className="text-xs opacity-70">{question.helper_text}</p>
                    )}
                    ...
                    {question.type === 'checkbox' && !question.multiple && (
                      <label className="flex items-center gap-2 text-sm">
                        <input
                          ...
                          disabled={disabled || readOnly}
                          checked={value === true}
                          onChange={(event) => onAnswerChange(question.key, event.target.checked)}
                        />
                        <span>{question.placeholder || 'Checked'}</span>
                      </label>
                    )}
                      ...
                      <div className="space-y-2">
                        {!readOnly && (
                          <input
                            ...
                            multiple={(question.upload_config?.max_files ?? 1) > 1 || question.multiple}
                        ...
                        {uploadingKey === question.key && (
                          <p className="text-xs font-semibold uppercase opacity-70">Uploading…</p>
                        )}
                        ...
                        {assetIds.length > 0 && uploadedAssets.length === 0 && (
                          <p className="text-xs opacity-70">
                            {assetIds.length} uploaded item{assetIds.length === 1 ? '' : 's'} attached.
                          </p>
                        )}
                      </div>
                    )}
