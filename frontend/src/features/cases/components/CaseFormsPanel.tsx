...
export default function CaseFormsPanel({
  ...
  const canShowAccessLink =
    Boolean(detail?.assignment.access_link_url)
    && Boolean(detail?.assignment.delivery_target)
    && usesEmailDelivery(detail.assignment.delivery_target as CaseFormDeliveryTarget);
  ...
  const handleCopyAccessLink = (): void => {
    if (!detail?.assignment.access_link_url) return;
