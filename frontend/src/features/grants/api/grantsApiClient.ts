...
export class GrantsApiClient {
  ...
  async getSummary(
    ...
    const response = await api.get<ApiEnvelope<GrantSummary>>('/grants/summary', {
      params: toQueryParams(query),
    });
    return unwrapApiData(response.data);
  }
  ...
  async listFunders(query: GrantListFilters = {}): Promise<PaginatedGrantResult<GrantFunder>> {
    return this.getPaginated<GrantFunder>('/grants/funders', query);
  }

  async getFunderById(id: string): Promise<GrantFunder> {
    return this.getSingle<GrantFunder>(`/grants/funders/${id}`);
  }

  async createFunder(payload: CreateGrantFunderDTO): Promise<GrantFunder> {
    const response = await api.post<ApiEnvelope<GrantFunder>>('/grants/funders', payload);
    return unwrapApiData(response.data);
  }

  async updateFunder(id: string, payload: UpdateGrantFunderDTO): Promise<GrantFunder> {
    const response = await api.put<ApiEnvelope<GrantFunder>>(`/grants/funders/${id}`, payload);
    return unwrapApiData(response.data);
  }

  async deleteFunder(id: string): Promise<void> {
    await api.delete(`/grants/funders/${id}`);
  }

  async listPrograms(query: GrantListFilters = {}): Promise<PaginatedGrantResult<GrantProgram>> {
    return this.getPaginated<GrantProgram>('/grants/programs', query);
  }

  async getProgramById(id: string): Promise<GrantProgram> {
    return this.getSingle<GrantProgram>(`/grants/programs/${id}`);
  }

  async createProgram(payload: CreateGrantProgramDTO): Promise<GrantProgram> {
    const response = await api.post<ApiEnvelope<GrantProgram>>('/grants/programs', payload);
    return unwrapApiData(response.data);
  }

  async updateProgram(id: string, payload: UpdateGrantProgramDTO): Promise<GrantProgram> {
    const response = await api.put<ApiEnvelope<GrantProgram>>(`/grants/programs/${id}`, payload);
    return unwrapApiData(response.data);
  }

  async deleteProgram(id: string): Promise<void> {
    await api.delete(`/grants/programs/${id}`);
  }
  ...
  async getRecipientById(id: string): Promise<RecipientOrganization> {
    return this.getSingle<RecipientOrganization>(`/grants/recipients/${id}`);
  }

  async createRecipient(payload: CreateRecipientOrganizationDTO): Promise<RecipientOrganization> {
    const response = await api.post<ApiEnvelope<RecipientOrganization>>('/grants/recipients', payload);
    return unwrapApiData(response.data);
  }
  ...
  async deleteRecipient(id: string): Promise<void> {
    await api.delete(`/grants/recipients/${id}`);
  }

  async listFundedPrograms(query: GrantListFilters = {}): Promise<PaginatedGrantResult<FundedProgram>> {
    return this.getPaginated<FundedProgram>('/grants/funded-programs', query);
  }

  async getFundedProgramById(id: string): Promise<FundedProgram> {
    return this.getSingle<FundedProgram>(`/grants/funded-programs/${id}`);
  }

  async createFundedProgram(payload: CreateFundedProgramDTO): Promise<FundedProgram> {
    const response = await api.post<ApiEnvelope<FundedProgram>>('/grants/funded-programs', payload);
    return unwrapApiData(response.data);
  }

  async updateFundedProgram(id: string, payload: UpdateFundedProgramDTO): Promise<FundedProgram> {
    const response = await api.put<ApiEnvelope<FundedProgram>>(`/grants/funded-programs/${id}`, payload);
    return unwrapApiData(response.data);
  }

  async deleteFundedProgram(id: string): Promise<void> {
    await api.delete(`/grants/funded-programs/${id}`);
  }

  async listApplications(query: GrantListFilters = {}): Promise<PaginatedGrantResult<GrantApplication>> {
    return this.getPaginated<GrantApplication>('/grants/applications', query);
  }

  async getApplicationById(id: string): Promise<GrantApplication> {
    return this.getSingle<GrantApplication>(`/grants/applications/${id}`);
  }

  async createApplication(payload: CreateGrantApplicationDTO): Promise<GrantApplication> {
    const response = await api.post<ApiEnvelope<GrantApplication>>('/grants/applications', payload);
    return unwrapApiData(response.data);
  }
  ...
  async listAwards(query: GrantListFilters = {}): Promise<PaginatedGrantResult<GrantAward>> {
    return this.getPaginated<GrantAward>('/grants/awards', query);
  }

  async getAwardById(id: string): Promise<GrantAward> {
    return this.getSingle<GrantAward>(`/grants/awards/${id}`);
  }

  async createAward(payload: CreateGrantAwardDTO): Promise<GrantAward> {
    const response = await api.post<ApiEnvelope<GrantAward>>('/grants/awards', payload);
    return unwrapApiData(response.data);
  }

  async updateAward(id: string, payload: UpdateGrantAwardDTO): Promise<GrantAward> {
    const response = await api.put<ApiEnvelope<GrantAward>>(`/grants/awards/${id}`, payload);
    return unwrapApiData(response.data);
  }

  async deleteAward(id: string): Promise<void> {
    await api.delete(`/grants/awards/${id}`);
  }

  async listDisbursements(query: GrantListFilters = {}): Promise<PaginatedGrantResult<GrantDisbursement>> {
    return this.getPaginated<GrantDisbursement>('/grants/disbursements', query);
  }

  async getDisbursementById(id: string): Promise<GrantDisbursement> {
    return this.getSingle<GrantDisbursement>(`/grants/disbursements/${id}`);
  }

  async createDisbursement(payload: CreateGrantDisbursementDTO): Promise<GrantDisbursement> {
    const response = await api.post<ApiEnvelope<GrantDisbursement>>('/grants/disbursements', payload);
    return unwrapApiData(response.data);
  }
  ...
  async deleteDisbursement(id: string): Promise<void> {
    await api.delete(`/grants/disbursements/${id}`);
  }

  async listReports(query: GrantListFilters = {}): Promise<PaginatedGrantResult<GrantReport>> {
    return this.getPaginated<GrantReport>('/grants/reports', query);
  }

  async getReportById(id: string): Promise<GrantReport> {
    return this.getSingle<GrantReport>(`/grants/reports/${id}`);
  }

  async createReport(payload: CreateGrantReportDTO): Promise<GrantReport> {
    const response = await api.post<ApiEnvelope<GrantReport>>('/grants/reports', payload);
    return unwrapApiData(response.data);
  }

  async updateReport(id: string, payload: UpdateGrantReportDTO): Promise<GrantReport> {
    const response = await api.put<ApiEnvelope<GrantReport>>(`/grants/reports/${id}`, payload);
    return unwrapApiData(response.data);
  }

  async deleteReport(id: string): Promise<void> {
    await api.delete(`/grants/reports/${id}`);
  }

  async listDocuments(query: GrantListFilters = {}): Promise<PaginatedGrantResult<GrantDocument>> {
    return this.getPaginated<GrantDocument>('/grants/documents', query);
  }

  async getDocumentById(id: string): Promise<GrantDocument> {
    return this.getSingle<GrantDocument>(`/grants/documents/${id}`);
  }

  async createDocument(payload: CreateGrantDocumentDTO): Promise<GrantDocument> {
    const response = await api.post<ApiEnvelope<GrantDocument>>('/grants/documents', payload);
    return unwrapApiData(response.data);
  }

  async updateDocument(id: string, payload: UpdateGrantDocumentDTO): Promise<GrantDocument> {
    const response = await api.put<ApiEnvelope<GrantDocument>>(`/grants/documents/${id}`, payload);
    return unwrapApiData(response.data);
  }

  async deleteDocument(id: string): Promise<void> {
    await api.delete(`/grants/documents/${id}`);
  }

  async listActivities(query: GrantListFilters = {}): Promise<PaginatedGrantResult<GrantActivityLog>> {
    return this.getPaginated<GrantActivityLog>('/grants/activities', query);
  }

  async getCalendar(query: CalendarQuery = {}): Promise<GrantCalendarItem[]> {
    const response = await api.get<ApiEnvelope<GrantCalendarItem[]>>('/grants/calendar', {
      params: toQueryParams(query),
    });
    return unwrapApiData(response.data);
  }
  ...
  async exportGrants(
    ...
    const response = await api.get<Blob>('/grants/export', {
