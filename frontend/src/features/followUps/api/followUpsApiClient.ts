...
export class FollowUpsApiClient {
  async fetchFollowUps(params: { filters?: FollowUpFilters; page?: number; limit?: number } = {}): Promise<{
    ...
    const endpoint = query.length > 0 ? `/follow-ups?${query}` : '/follow-ups';
  ...
  async cancelFollowUp(followUpId: string, data?: CompleteFollowUpDTO): Promise<FollowUp> {
    const response = await api.post<FollowUp>(`/follow-ups/${followUpId}/cancel`, data || {});
    return response.data;
  }
  ...
  async fetchFollowUpSummary(filters?: FollowUpFilters): Promise<FollowUpSummary> {
    ...
    const query = queryParams.toString();
    const endpoint = query.length > 0 ? `/follow-ups/summary?${query}` : '/follow-ups/summary';
    const response = await api.get<FollowUpSummary>(endpoint);
    return response.data;
  }

  async fetchUpcomingFollowUps(limit = 10): Promise<FollowUpWithEntity[]> {
    const response = await api.get<FollowUpWithEntity[]>('/follow-ups/upcoming', { params: { limit } });
    return response.data;
  }
}
