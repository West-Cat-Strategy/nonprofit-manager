...
const AlertConfigModal = ({ config, onClose, onSuccess }: AlertConfigModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-app-surface rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-app-text-muted mb-1">Metric *</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-app-text-muted mb-1">Condition *</label>
              <select
          ...
          {['exceeds', 'drops_below'].includes(formData.condition) && (
            <div>
              <label className="block text-sm font-medium text-app-text-muted mb-1">Threshold *</label>
              <input
                type="number"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-app-text-muted mb-1">Severity *</label>
              <select
            ...
            <div className="grid grid-cols-2 gap-3">
              {(['email', 'in_app', 'slack', 'webhook'] as AlertChannel[]).map((channel) => (
                <label key={channel} className="flex items-center space-x-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-app-text-muted capitalize">{channel.replace('_', ' ')}</span>
                </label>
              ))}
            </div>
