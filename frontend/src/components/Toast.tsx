...
const Toast = ({ id, message, type, duration = 5000, onClose }: ToastProps) => {
  ...
  const getToastStyles = () => {
    switch (type) {
      case 'success':
        return 'bg-app-accent-soft border-app-border text-app-accent-text';
      case 'error':
        return 'bg-app-accent-soft border-app-border text-app-accent-text';
      case 'warning':
        return 'bg-app-accent-soft border-app-border text-app-accent-text';
      case 'info':
        return 'bg-app-accent-soft border-app-accent-soft text-app-accent-text';
      default:
        return 'bg-app-surface-muted border-app-border text-app-text';
    }
  };
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => onClose(id)}
        className="flex-shrink-0 text-app-text-subtle hover:text-app-text-muted transition"
